...
/// @note Intersteps are special variables that are only defined at points 
/// within a mesh interval (and *not*on the mesh points) for a particular 
/// transcription scheme; therefore, intersteps are NaN at the start and end 
/// time points and either NaN or a real value at the other time points.
/// @verbatim
/// num_states=<number-of-state-variables>
/// num_controls=<number-of-control-variables>
/// num_adjuncts=<number-of-adjunct-variables>
/// num_intersteps=<number-of-interstep-variables>
/// num_parameters=<number-of-parameter-variables>
/// time,<s0-name>,...,<c0-name>,...,<a0-name>,...,<i0-name>,...,<p0-name>,...
/// <#>,<#>,...,<#>,...,<#>,...,  <NaN>   ,...,<#>,...
/// <#>,<#>,...,<#>,...,<#>,...,<#-or-NaN>,...,<NaN>,...
///  : , : ,..., : ,..., : ,...,    :     ,...,  :  ,...
/// <#>,<#>,...,<#>,...,<#>,...,  <NaN>   ,...,<NaN>,...
...
struct Iterate {
    ...
    Eigen::MatrixXd intersteps;
    Eigen::VectorXd parameters;
    std::vector<std::string> state_names;
    std::vector<std::string> control_names;
    std::vector<std::string> adjunct_names;
    std::vector<std::string> interstep_names;
    ...
    bool empty() const {
        return !(time.size() || states.size() || controls.size() ||
                adjuncts.size() || intersteps.size() || parameters.size() || 
                state_names.size() || control_names.size() || 
                adjunct_names.size() || interstep_names.size() ||

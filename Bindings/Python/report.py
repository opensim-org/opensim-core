...
class Report(object):
    def __init__(self,
        ...
        # Check that the colorlist is the correct length
        if len(self.colorlist) != len(self.refs)+1:
            raise Exception(f"The argument 'colorlist' should have length equal "
                            f"to the number of files provided (trajectory and "
                            f"reference files), but has length "
                            f"{len(self.colorlist)}.")
        ...
        for i, key in enumerate(var_dict.keys()):
            ...
            for path, ls in zip(var_dict[key], ls_dict[key]):
                ...
                plt.plot(self.time, var, ls=ls, 
                         color=self.colorlist[len(self.refs)],
                         linewidth=1.5)

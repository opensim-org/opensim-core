...
public:
    ...
    void setQuaternionToZeroRotation()  {Vec<4,P>::operator=( Vec<4,P>(1,0,0,0) );}
    ...
    void setQuaternionToNaN()           {Vec<4,P>::setToNaN();}
    ...
    SimTK_SimTKCOMMON_EXPORT void  setQuaternionFromAngleAxis(const Vec<4,P>& av);
    ...
    SimTK_SimTKCOMMON_EXPORT void  setQuaternionFromAngleAxis(const P& a, const UnitVec<P,1>& v);

    /// Returns [ a vx vy vz ] with (a,v) in canonical form, i.e., 
    /// -180 < a <= 180 and |v|=1. 
    SimTK_SimTKCOMMON_EXPORT Vec<4,P> convertQuaternionToAngleAxis() const;

    /// Zero-cost cast of a Quaternion_ to its underlying Vec4; this is \e not
    /// converted to axis-angle form.
    const Vec<4,P>& asVec4() const  {return *static_cast<const Vec<4,P>*>(this);}
    ...
    Quaternion_& normalizeThis() { 
        const P epsilon = std::numeric_limits<P>::epsilon();
        const P magnitude = Vec<4,P>::norm();
    ...
    Quaternion_(const Vec<4,P>& v, bool) : Vec<4,P>(v) {}

//----------------------------------------------------------------------------------------------------
// The following code is obsolete - it is here temporarily for backward compatibility (Mitiguy 10/13/2007)
//----------------------------------------------------------------------------------------------------
private:
    Vec<4,P> convertToAngleAxis() const                          { return convertQuaternionToAngleAxis();}
    void setToAngleAxis(const Vec<4,P>& av)                      { setQuaternionFromAngleAxis(av);}
    void setToAngleAxis(const P& a, const UnitVec<P,1>& v){ setQuaternionFromAngleAxis(a,v);}

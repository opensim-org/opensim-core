...
//
//     problem.setTimeBounds(0, 1);
//     problem.setStateInfo("/jointset/j0/q0/value", {-0.6 * pi, 0.6 * pi});
//     problem.setStateInfo("/jointset/j0/q0/speed", {-10, 10});
//     problem.setStateInfo("/jointset/j1/q1/value", {0, pi});
//     problem.setStateInfo("/jointset/j1/q1/speed", {-10, 10});
//     problem.setControlInfo("/tau0", {-20, 20});
//     problem.setControlInfo("/tau1", {-20, 20});
//
//     auto testData = std::make_shared<AccelerationsAndJointReaction>();
//     problem.addGoal<MocoMultiplierAccelerationGoal>("jointset/j1", testData);
//
//     auto& solver = study.initCasADiSolver();
//     int N = 5;
//     solver.set_num_mesh_intervals(N);
...
//     int Nguess = 2;
//     guess.resampleWithNumTimes(Nguess);
// const double q0 = 0.25 * pi;
// const double q1 = 0.5 * pi;
// guess.setState("/jointset/j0/q0/value",
//             createVectorLinspace(Nguess, q0, q0));
//     guess.setState("/jointset/j1/q1/value",
//             createVectorLinspace(Nguess, q1, q1));
//     const double u0 = 0; // TODO 2.471;
//     guess.setState("/jointset/j0/q0/speed",
//             createVectorLinspace(Nguess, u0, u0));
//     const double u1 = 0; // TODO -1.61;
//     guess.setState("/jointset/j1/q1/speed",
//             createVectorLinspace(Nguess, u1, u1));
//     const double lambda0 = 0.813604;
//     guess.setMultiplier("lambda_cid2_p0",
//             createVectorLinspace(Nguess, lambda0, lambda0));
//     const double lambda1 = 1.390461;
//     guess.setMultiplier("lambda_cid2_p1",
//             createVectorLinspace(Nguess, lambda1, lambda1));
//     const double accel0 = .4024742;
//     guess.setDerivative("/jointset/j0/q0/accel",
//             createVectorLinspace(Nguess, accel0, accel0));
//     const double accel1 = 0.351;
//     guess.setDerivative("/jointset/j1/q1/accel",
//             createVectorLinspace(Nguess, accel1, accel1));
//
//     solver.setGuess(guess);
//
//     MocoSolution solution = study.solve().unseal();
...
//     // supplied, not any computed by Simbody.
//
//     CHECK(solution.getMultiplier("lambda_cid2_p0").getElt(0, 0) ==
//             Approx(lambda0));
//     CHECK(solution.getMultiplier("lambda_cid2_p1").getElt(0, 0) ==
//             Approx(lambda1));
//     CHECK(solution.getDerivative("/jointset/j0/q0/accel").getElt(0, 0) ==
//             Approx(accel0));
//     CHECK(solution.getDerivative("/jointset/j1/q1/accel").getElt(0, 0) ==
//             Approx(accel1));
//
//     CHECK(testData->udot[0] == Approx(accel0));
//     CHECK(testData->udot[1] == Approx(accel1));
//
//     double c0 = cos(q0);
//     double c01 = cos(q0 + q1);
//     double s0 = sin(q0);
//     double s01 = sin(q0 + q1);
//     double ax = -u0 * u0 * c0 - accel0 * s0 -
//                 0.5 * (u0 + u1) * (u0 + u1) * c01 -
//                 0.5 * (accel0 + accel1) * s01;
//     double ay = -u0 * u0 * s0 + accel0 * c0 -
//                 0.5 * (u0 + u1) * (u0 + u1) * s01 +
//                 0.5 * (accel0 + accel1) * c01;
//     double m = 1.0;
//     double g = model->get_gravity().norm();
//     std::cout << "DEBUGa " << ax << " " << ay << std::endl;
//     std::cout << "DEBUG " << m * ay << " " << m * g << std::endl;
//     CAPTURE(m * ax);
//     CAPTURE(lambda1);
//     double Rx = m * ax - lambda1;
//     CAPTURE(m * ay);
//     CAPTURE(m * g);
//     double Ry = m * ay + m * g;
//     double Rz = -lambda0;
//     double Mx = -lambda0 * s01;
//     double My = lambda0 * c01;
//     // TODO: acceleration.
//     double Mz = -0.5 * c01 * m * g - lambda1 * s01 + 0;
//
//
//     std::cout << "DEBUG "
//         << Mx << " " << My << " " << Mz << " "
//         << Rx << " " << Ry << " " << Rz << " "
//         << testData->reaction << std::endl;
//
//     CAPTURE(c0);
//     CAPTURE(s0);
//     CAPTURE(c01);

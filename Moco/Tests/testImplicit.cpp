...
//            "targetframe", model.getGround(), SimTK::Transform(Vec3(0, 2,
//            0)));
...
//}
//
// TEMPLATE_TEST_CASE("Similar solutions between implicit and explicit
// dynamics",
...
//        MocoTrajectory iter(createVectorLinspace(6, 0, 1), {}, {}, {}, {"a",
//        "b"},
//                {}, {}, {}, {}, {6, 2, 0.5}, {});
...
//        MocoTrajectory iterA(createVectorLinspace(6, 0, 1), {}, {}, {}, {"a",
//        "b"},
//                {}, {}, {}, {}, {6, 2, valueA}, {});
//        MocoTrajectory iterB(createVectorLinspace(6, 0, 1), {}, {}, {}, {"a",
//        "b"},
//                {}, {}, {}, {}, {6, 2, valueB}, {});
...
public:
    ...
    OpenSim_DECLARE_INPUT(
            implicitderiv_fiber_length, double, SimTK::Stage::Dynamics, "TODO");
    ...
    double getImplicitResidualFiberLength(const SimTK::State& s) const {
        ...
        if (!isCacheVariableValid(s, "implicitresidual_fiber_length")) {
            // Get the fiber derivative control value. This is NOT equal to the
            // value returned by getStateVariableDerivativeValue().
            ...
            // Get the fiber lenth state variable value.
            ...
            setCacheVariableValue(s, "implicitresidual_fiber_length", residual);
            markCacheVariableValid(s, "implicitresidual_fiber_length");
        }
        return getCacheVariableValue<double>(
                s, "implicitresidual_fiber_length");
    }
...
TEST_CASE("Auxiliary implicit dynamics") {
    SECTION("State unit tests") {
        ...
        SimTK::Function::Polynomial polyFunc(
                SimTK::Vector(SimTK::Vec3(0, 0, 1)));
        auto time = createVectorLinspace(20, 0, 1);
        for (int i = 0; i < time.size(); ++i) {
            // Set the velocity control discrete variable.
            const double velocityControl =
                    polyFunc.calcValue(SimTK::Vector(1, time[i]));
            implicit_auxdyn->setDiscreteVariableValue(
                    state, "implicitderiv_fiber_length", velocityControl);
            model.realizeDynamics(state);

            double fiberLength = implicit_auxdyn->getStateVariableValue(
                    state, "fiber_length");
            double fiberVelocity =
                    implicit_auxdyn->getStateVariableDerivativeValue(
                            state, "fiber_length");
            // y y' = 1
            double residual = fiberLength * velocityControl - 1;

            // The fiber length returned by the component matches the state
            // value.
            CHECK(state.getY()[0] == fiberLength);
            // The residual return by the component represents the dynamics.
            CHECK(residual == implicit_auxdyn->getOutputValue<double>(
                                      state, "implicitresidual_fiber_length"));
            // The state variable derivative represents the dynamics.
            CHECK(fiberVelocity == 1.0 / fiberLength);
        }
    }
    ...
    //    problem.setStateInfo("/implicit_auxdyn/fiber_length", {}, 1.0);

...
public:
    ...
    /// and max control are used for the bounds. If the control is associated
    /// with a non-scalar actuator (i.e. uses multiple control variables), then 
    /// the control name will be the actuator path appended by the control index
    /// (e.g. "/actuator_0");

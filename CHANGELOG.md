...
- Fixed bug and speed issue with `model.setStateVariableValues()` caused by enforcing constraints after each coordinate value was being set (PR #1911). Removing the automatic enforcement of constraints makes setting all state values much faster, but also requires calling `model.assemble()` afterwards. Enforcing constraints after setting each coordinate value individually was also incorrect, since it neglected the effect of other coordinate changes have on the current coordinate. For example, take two identical blocks sliding on the same axis (1dof each), and apply a constraint that requires a face on each block to be welded together and that is satisfied when both blocks are at a coordinate value of 0, respectively. If both blocks are being adjusted together they will reassemble where you put them, but if you set block 1 to +1m along its coordinate, the constraint will be violated and cause both blocks to be moved by +0.5m to minimize the coordinate changes. Then moving block 2 to 1m will cause the assembly to split the difference with block 1 (now at 0.5m) and now both blocks will end up at 0.75 for their coordinate values and not 1, had both values been set prior to enforcement of the constraint.

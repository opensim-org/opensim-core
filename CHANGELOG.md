...
v4.2
====
- Fixed a bug with InverseDynamicsTool/InverseDynamicsSolver to account for cases where a model has extra slots in its `State`'s "q" ([PR #2971](https://github.com/opensim-org/opensim-core/pull/2971))
...
- Add the ActivationCoordinateActuator component, which is a CoordinateActuator with simple activation dynamics ([PR #2699](https://github.com/opensim-org/opensim-core/pull/2699)).
...
- Minor performance improvements (5-10 %) for controller-heavy models ([PR #2806](https://github.com/opensim-org/opensim-core/pull/2806))
...
- Fixed a critical bug in Induced Accelerations Analysis which prevents analysis to run when external forces are present ([PR #2847](https://github.com/opensim-org/opensim-core/pull/2847)).
- For PrescribedController, the controls_file column labels can now be absolute paths to actuators (previously, the column labels were required to be actuator names).
...
- Removed `Path` abstract base class ([PR #2844](https://github.com/opensim-org/opensim-core/pull/2844))
  - Unused by OpenSim and related projects
- Improved the performance of `ComponentPath` ([PR #2844](https://github.com/opensim-org/opensim-core/pull/2844))
...
- Refactored InverseKinematicsSolver to allow for adding (live) Orientation data to track, introduced BufferedOrientationsReference to queue data ([PR #2855](https://github.com/opensim-org/opensim-core/pull/2855))
- `opensim.log` will only be created/opened when the first message is logged to it ([PR #2880](https://github.com/opensim-org/opensim-core/pull/2880)):
  - Previously, `opensim.log` would always be created, even if nothing was logged
- Added a CMake option, `OPENSIM_DISABLE_LOG_FILE` ([PR #2880](https://github.com/opensim-org/opensim-core/pull/2880)):
...
- Fix bug in error reporting of sensor tracking ([PR #2893](https://github.com/opensim-org/opensim-core/pull/2893))
- Throw an exception rather than log an error message when an unrecognized type is encountered in xml/osim files ([PR #2914](https://github.com/opensim-org/opensim-core/pull/2914))
- Added ScapulothoracicJoint as a builtin Joint type instead of a plugin (PRs #2877 and #2932)

v4.1
====
- Added `OrientationsReference` as the frame orientation analog to the location of experimental markers. Enables experimentally measured orientations from wearable sensors (e.g. from IMUs) to be tracked by reference frames in the model. A correspondence between the experimental (IMU frame) orientation column label and that of the virtual frame on the `Model` is expected. The `InverseKinematicsSolver` was extended to simultaneously track the `OrientationsReference` if provided. ([PR #2412](https://github.com/opensim-org/opensim-core/pull/2412))
- Removed the undocumented `bool dumpName` argument from `Object::dump()` and made the method `const` so it can be safely called on `const` objects. ([PR #2412](https://github.com/opensim-org/opensim-core/pull/2412))
...
- Methods that operate on SimTK::Vec<n> are now available through Java/Matlab and python bindings to add/subtract/divide/multiply vec<n> contents with a scalar ([PR #2558](https://github.com/opensim-org/opensim-core/pull/2558))
- The new Stopwatch class allows C++ API users to easily measure the runtime of their code.
- If finalizeConnections() method was not called on a model after making changes and before printing, an exception is thrown to avoid creating corrupt model files quietly ([PR #2529](https://github.com/opensim-org/opensim-core/pull/2529))
- Updated the docopt.cpp dependency so that OpenSim can be compiled with Visual C++ from Visual Studio 2019.
- Added `Blankevoort1991Ligament` force component which represents ligament fibers as non-linear path springs. The force-strain curve has a quadratic toe region at low strains and a linear stiffness region at high strains. ([PR #2632](https://github.com/opensim-org/opensim-core/pull/2632))
...
Bug Fixes
---------
- Fixed bug in osimTable2Struct.m for renaming unlabelled markers ([PR #2491](https://github.com/opensim-org/opensim-core/pull/2491))
- Fixed bug that resulted in an exception when reading C3D files without forces. Now, if the C3D doesn't contain markers or forces, an empty table will be returned ([PR #2421](https://github.com/opensim-org/opensim-core/pull/2421))
...
- OpenSim no longer supports model file formats predating version 1.8.1 ([PR #2498](https://github.com/opensim-org/opensim-core/pull/2498))
...
Other Changes
-------------
- Performance of reading large data files has been significantly improved. A 50MB .sto file would take 10-11 min to read now takes 2-3 seconds. ([PR #2399](https://github.com/opensim-org/opensim-core/pull/2399))
...
- Added Python example that demonstrates how to run an optimization using the cma package and how to avoid an expensive call to `initSystem()` within the objective function. ([PR #2604](https://github.com/opensim-org/opensim-core/pull/2604))
...
- Component naming is more strictly enforced and names with spaces are no longer accepted. Spaces are only allowable as separators for `Output` or `Channel` names that satisfy a list `Input`. ([PR #1955](https://github.com/opensim-org/opensim-core/pull/1955))
- The Actuator class has been renamed to ScalarActuator (and `Actuator_` has been renamed to `Actuator`) ([PR #126](https://github.com/opensim-org/opensim-core/pull/126)).
  If you have subclassed from Actuator, you must now subclass from ScalarActuator.
- Methods like `Actuator::getForce` are renamed to use "Actuator" instead (e.g., `Actuator::getActuator`) ([PR #209](https://github.com/opensim-org/opensim-core/pull/209)).
- Markers are now ModelComponents ([PR #188](https://github.com/opensim-org/opensim-core/pull/188)). Code is included for conversion on serialization/de-serialization.
- MarkerSet::addMarker() was removed ([PR #1898](https://github.com/opensim-org/opensim-core/pull/1898)). Please use Model::addMarker() to add markers to your model.
...
- `Model::replaceMarkerSet()` was removed. ([PR #1938](https://github.com/opensim-org/opensim-core/pull/1938)) Please use `Model::updMarkerSet()` to edit the model's MarkerSet instead.
- The argument list for `Model::scale()` was changed: the `finalMass` and
  ...
  3.3 was `false`. ([PR #1994](https://github.com/opensim-org/opensim-core/pull/1994))
...
Model. ([PR #1948](https://github.com/opensim-org/opensim-core/pull/1948))
...
Bug Fixes
---------
- Fixed a typo in one of the method names for AbstractTool verifyUniqueComulnLabels -> verifyUniqueColumnLabels ([PR #130](https://github.com/opensim-org/opensim-core/pull/130))
- Fixed bug where Body VisibleObject was not serialized when writing a model to XML ([PR #139](https://github.com/opensim-org/opensim-core/pull/139))
- Fixed memory leaks in AssemblySolver and using Simtk::XML ([PR #176](https://github.com/opensim-org/opensim-core/pull/176))
- Fixed model mass scaling. When 'preserve mass distribution' is unchecked (GUI) the input mass was previously not respected and the resulting scaled model mass does not equal the input mass. The modelscaler() now alters the body masses and inertias to match the input mass. ([PR #230](https://github.com/opensim-org/opensim-core/pull/230))
- Fixed a bug in the equilibrium solution of Millard and Thelen muscles, where the initial activation and fiber-length values (for solving for equilibrium) were always coming from the default values. This was unnecessary, because unless specified otherwise, the state automatically contains the default values. This fixes an issue where initial states activations from a file were not respected by the Forward Tool and instead, the initial activations would revert to the model defaults. ([PR #272](https://github.com/opensim-org/opensim-core/pull/272))
- Fixed a bug where MuscleAnalysis was producing empty moment arm files. We now avoid creating empty Moment and MomentArm storage files when `_computeMoments` is False. ([PR #324](https://github.com/opensim-org/opensim-core/pull/324))
- Fixed bug causing the muscle equilibrium solve routine in both Thelen2003Muscle and Millard2012EquilibriumMuscle to fail to converge and erroneously return the minimum fiber length. The fix added a proper reduction in step-size when errors increase and limiting the fiber-length to its minimum. ([PR #1728](https://github.com/opensim-org/opensim-core/pull/1728))
- Fixed a bug where Models with Bodies and Joints (and other component types) with the same name were loaded without error. Duplicately named Bodies were simply being ignored and only the first Body of that name in the BodySet was being used, for example, to connect a Body to its parent via its Joint, or to affix path points to its respective Body. Now, duplicate names are flagged and renamed so they are uniquely identified. ([PR #1887](https://github.com/opensim-org/opensim-core/pull/1887))
- Fixed bug and speed issue with `model.setStateVariableValues()` caused by enforcing constraints after each coordinate value was being set ([PR #1911](https://github.com/opensim-org/opensim-core/pull/1911)). Removing the automatic enforcement of constraints makes setting all state values much faster, but also requires calling `model.assemble()` afterwards. Enforcing constraints after setting each coordinate value individually was also incorrect, since it neglected the effect of other coordinate changes have on the current coordinate. All coordinate values must be set before enforcing constraints.
- Fixed a bug that resulted in incorrect Ligament resting lengths after scaling.
  ([PR #1994](https://github.com/opensim-org/opensim-core/pull/1994))

New Classes
-----------
- Added a BodyActuator component, which applies a spatial force on a specified Point of a Body ([PR #126](https://github.com/opensim-org/opensim-core/pull/126))
...
- Connections to Bodies upgraded to PhysicalFrames and locations on these frames are now represented by PhysicalOffsetFrame ([PR #370](https://github.com/opensim-org/opensim-core/pull/370))
- Joints were refactored so that the base Joint manages the parent and child frame connections, including the definition of local PhysicalOffsetFrames to handle offsets defined as separate location and orientation properties. ([PR #589](https://github.com/opensim-org/opensim-core/pull/589))
- The WeldConstraint and BushingForces (BushingForce, CoupledBushingForce, FunctionBasedBushingForce, and ExpressionBasedBushingForce) were similarly unified (like Joints) to handle the two Frames that these classes require to operate. A LinkTwoFrames intermediate class was introduced to house the common operations. Convenience constructors for WeldConstraint and BushingFrames were affected and now require the name of the Component as the first argument. ([PR #649](https://github.com/opensim-org/opensim-core/pull/649))
- The new StatesTrajectory class allows users to load an exact representation of previously-computed states from a file. ([PR #730](https://github.com/opensim-org/opensim-core/pull/730))
- Added Point as a new base class for all points, which include: Station, Marker, and PathPoints

- Added OutputReporter as an Analysis so that users can use the existing AnalyzeTool and ForwardTool to extract Output values of interest, without modifications to the GUI. ([PR #1991](https://github.com/opensim-org/opensim-core/pull/1991))
...
Python ([PR #930](https://github.com/opensim-org/opensim-core/pull/930)).
- Changed wrapping of `SimTK::Array_<OpenSim::CoordinateReference>` from `ArrayCoordinateReference` to `SimTKArrayCoordinateReference` for consistency with other classes. ([PR #1842](https://github.com/opensim-org/opensim-core/pull/1842))
...
- There is now a formal CMake mechanism for using OpenSim in your own C++
  project. See cmake/SampleCMakeLists.txt. ([PR #187](https://github.com/opensim-org/opensim-core/pull/187))
- Substantial cleanup of the internal CMake scripts.
- Lepton was upgraded to the latest version ([PR #349](https://github.com/opensim-org/opensim-core/pull/349))
- Made Object::print a const member function ([PR #191](https://github.com/opensim-org/opensim-core/pull/191))
- Improved the testOptimization/OptimizationExample to reduce the runtime ([PR #416](https://github.com/opensim-org/opensim-core/pull/416))
...
- Thelen2003Muscle, Millard2012EquilibriumMuscle, and
  ...
  calculation fails to converge ([PR #1201](https://github.com/opensim-org/opensim-core/pull/1201)).
- Thelen2003Muscle and Millard2012EquilibriumMuscle no longer clamp excitations (i.e. controls)
  ...
  specific bounds of Muscle models. ([PR #1548](https://github.com/opensim-org/opensim-core/pull/1548))
...
- The `scale()` method was removed from the `SimbodyEngine` class (the contents
  were moved into `Model::scale()`). ([PR #1994](https://github.com/opensim-org/opensim-core/pull/1994))
- Any class derived from ModelComponent can now add its own implementation of
  ...
  its properties are updated during scaling. ([PR #1994](https://github.com/opensim-org/opensim-core/pull/1994))

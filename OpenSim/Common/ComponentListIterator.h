...
ComponentListIterator<T>& ComponentListIterator<T>::operator++() {
    ...
    // If processing a subtree under _root we stop when our successor is the
    // same as the successor of _root as this indicates we're leaving the
    // _root's subtree.
...
void ComponentListIterator<T>::advanceToNextValidComponent() {
    ...
    while (_node != nullptr &&
            (dynamic_cast<const T*>(_node) == nullptr || !_filter.isMatch(*_node)))
    {
        if (_node->_components.size() > 0)
            _node = _node->_components[0];
        else {

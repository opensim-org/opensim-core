...
*/
template<class T, bool> class ArrayPtrsIterator;
...
*/
template<class T, bool is_const_iterator = true>
class ArrayPtrsIterator : public std::iterator<std::forward_iterator_tag, T>
{
    /**
    * For const_iterator:   define ArrayPointerType to be a   pointer to a const pointer to T
    * For regular iterator: define ArrayPointerType to be a   T**
    */
    typedef typename std::conditional<is_const_iterator, T* const *, T**>::type ArrayPointerType;
...
public:
    ...
    */
    ArrayPtrsIterator(ArrayPointerType ptr)
        : pointer(ptr)
    {
    }
    ...
    * a const_iterator.
    */
    ArrayPtrsIterator(const ArrayPtrsIterator<T, false>& rhs)
        : pointer(rhs.pointer)
    {
    }
    ...
    */
    bool operator==(const ArrayPtrsIterator& rhs) const
    {
    ...
    */
    ArrayPtrsIterator operator++(int)
    {
        const ArrayPtrsIterator old(*this);
        ++(*this);
        return old;
    }

    /**
    * Make ArrayPtrsIterator<true> a friend class of ArrayPtrsIterator<false>
    * so the copy constructor can access the private member variables.
    */
    friend class ArrayPtrsIterator<T, true>;
    
private:
    ArrayPointerType pointer;
}; // end of nested class ArrayPtrsIterator

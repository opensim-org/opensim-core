 ...
 * Destructor.
 */
DecompHardUTWalk::~DecompHardUTWalk()
{
 ...
 * rdUTWalking8 model.
 */
DecompHardUTWalk::DecompHardUTWalk(rdUTWalking8 *aModel) :
	DecompHard(aModel)
{
	setNull();
		...
		for(j=0;j<3;j++) {
			...
			if(whichC==0) {
				...
				*c = _bc[i].getPC(j)->evaluateC2(fsAcc);
				...
				printf("DecompHardUTWalk.suComputeConstraint: ERROR- invalid C\n");
			}

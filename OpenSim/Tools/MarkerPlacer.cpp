...
bool MarkerPlacer::processModel(
    ...
    for (int i = 0; i < get_IKTaskSet().getSize(); i++) {
        auto* coordTask = dynamic_cast<IKCoordinateTask*>(&get_IKTaskSet()[i]);
    ...
    if(_printResultFiles) {
        ...
        if (detail::isValidFileName(getProperty_output_marker_file())) {
        ...
        if (detail::isValidFileName(getProperty_output_motion_file())) {

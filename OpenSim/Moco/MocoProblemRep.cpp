...
void MocoProblemRep::initialize() {
    ...
    for (SimTK::ConstraintIndex cid(0); cid < NC; ++cid) {
        ...
        if (!constraint.isDisabled(state)) {
            ...
            for (int i = 0; i < mp; ++i) {
                std::string name = fmt::format("cid{}_p{}",static_cast<int>(cid), i);
            ...
            for (int i = 0; i < mv; ++i) {
                std::string name = fmt::format("cid{}_v{}", static_cast<int>(cid), i);
                MocoVariableInfo info("lambda_" + name, multBounds,
                        multInitBounds, multFinalBounds);
            ...
            for (int i = 0; i < ma; ++i) {
                std::string name = fmt::format("cid{}_a{}", static_cast<int>(cid), i);
                MocoVariableInfo info("lambda_" + name, multBounds,
                        multInitBounds, multFinalBounds);

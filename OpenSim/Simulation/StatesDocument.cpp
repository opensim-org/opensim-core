...
struct SDocUtil {
    ...
    appendVarElt(const string& path, const string& tag, const string& type,
        const SimTK::Array_<T>& valArr, Element& parent, int precision)
    {
    ...
    initializeStatesForStateVariable(Element& varElt, const Model& model,
        const string& path, SimTK::Array_<SimTK::State> & traj)
    {
...
StatesDocument::
StatesDocument(const Model& model, const SimTK::Array_<SimTK::State>& trajectory,
...
//_____________________________________________________________________________
StatesDocument::
StatesDocument(const Model& model, const vector<SimTK::State>& trajectory,
    const SimTK::String& note, int p)
{
    this->note = note;
    this->precision = clamp(1, p, SimTK::LosslessNumDigitsReal);
    ...
    vector<SimTK::State>& trajectoryNonconst = const_cast<vector<SimTK::State>&>(trajectory);
...
void
StatesDocument::
formDoc(const Model& model, const SimTK::Array_<SimTK::State>& traj) {
...
void
StatesDocument::
formRootElement(const Model& model, const SimTK::Array_<SimTK::State>& traj) {
...
void
StatesDocument::
formNoteElement(const Model& model, const SimTK::Array_<SimTK::State>& traj) {
...
void
StatesDocument::
formTimeElement(const Model& model, const SimTK::Array_<SimTK::State>& traj) {
    // Form time element.
...
void
StatesDocument::
formContinuousElement(const Model& model, const SimTK::Array_<SimTK::State>& traj) {
...
void
StatesDocument::
formDiscreteElement(const Model& model, const SimTK::Array_<SimTK::State>& traj) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (SimTK::Value<bool>::isA(v)) {
            ...
            model.getDiscreteVariableTrajectory<SimTK::Vec2>(
                paths[i], traj, vArr);
            SDocUtil::appendVarElt<SimTK::Vec2>(paths[i], "variable", "Vec2",
                vArr, discreteElt, precision);
        }
            ...
            model.getDiscreteVariableTrajectory<SimTK::Vec3>(
                paths[i], traj, vArr);
            SDocUtil::appendVarElt<SimTK::Vec3>(paths[i], "variable", "Vec3",
                vArr, discreteElt, precision);
        }
            ...
            model.getDiscreteVariableTrajectory<SimTK::Vec4>(
                paths[i], traj, vArr);
            SDocUtil::appendVarElt<SimTK::Vec4>(paths[i], "variable", "Vec4",
                vArr, discreteElt, precision);
        }
            ...
            model.getDiscreteVariableTrajectory<SimTK::Vec5>(
                paths[i], traj, vArr);
            SDocUtil::appendVarElt<SimTK::Vec5>(paths[i], "variable", "Vec5",
                vArr, discreteElt, precision);
        }
            ...
            model.getDiscreteVariableTrajectory<SimTK::Vec6>(
                paths[i], traj, vArr);
            SDocUtil::appendVarElt<SimTK::Vec6>(paths[i], "variable", "Vec6",
                vArr, discreteElt, precision);
        }
        else {
...
void
StatesDocument::
formModelingElement(const Model& model, const SimTK::Array_<SimTK::State>& traj) {
    // Form continuous element.
...
void
StatesDocument::
deserialize(const Model& model, SimTK::Array_<SimTK::State>& traj) {
...
void
StatesDocument::
deserialize(const Model& model, vector<SimTK::State>& trajectory) {
    checkDocConsistencyWithModel(model);
    initializeNumberOfStateObjects();
    initializePrecision();
    initializeNote();
...
void
StatesDocument::
prepareStatesTrajectory(const Model& model, SimTK::Array_<SimTK::State>& traj) {
...
void
StatesDocument::
prepareStatesTrajectory(const Model& model, vector<SimTK::State>& traj) {
    // Create a local copy of the Model and get its default State.
    Model localModel(model);
    SimTK::State defaultState = localModel.initSystem();
...
void
StatesDocument::
initializeTime(SimTK::Array_<SimTK::State>& traj) {
    // Find the element
    Element rootElt = doc.getRootElement();
...
void
StatesDocument::
initializeContinuousVariables(const Model& model, SimTK::Array_<SimTK::State>& traj) {
...
void
StatesDocument::
initializeDiscreteVariables(const Model& model, SimTK::Array_<SimTK::State>& traj) {
    Element rootElt = doc.getRootElement();
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (type == "bool") {
            ...
            SDocUtil::initializeStatesForDiscreteVariable<SimTK::Vec2>(varElts[i],
                model, path, traj);
        }
        else if(type == "Vec3") {
            SDocUtil::initializeStatesForDiscreteVariable<SimTK::Vec3>(varElts[i],
                model, path, traj);
        }
        else if(type == "Vec4") {
            SDocUtil::initializeStatesForDiscreteVariable<SimTK::Vec4>(varElts[i],
                model, path, traj);
        }
        else if(type == "Vec5") {
            SDocUtil::initializeStatesForDiscreteVariable<SimTK::Vec5>(varElts[i],
                model, path, traj);
        }
        else if(type == "Vec6") {
            SDocUtil::initializeStatesForDiscreteVariable<SimTK::Vec6>(varElts[i],
                model, path, traj);
        }
        else {
            string msg = "Unrecognized type: " + type;
...
void
StatesDocument::
initializeModelingOptions(const Model& model, SimTK::Array_<SimTK::State>& traj) {
    // Find the element
    Element rootElt = doc.getRootElement();

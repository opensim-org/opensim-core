...
OpenSim_DECLARE_LIST_PROPERTY_ATMOST(axes, std::string, 3,
        "Axes (X Y Z) along which to scale a body. For example, 'X Y Z'"
        "scales along all three axes, and 'Y' scales just along the Y axis.")
...
public:
    ...
    const Array<std::string>& getAxisNames() const {
        static Array<std::string> _axes{get_axes(0), get_axes(1), get_axes(2)};
        return _axes;
    }
#endif
    Array<std::string>& getAxisNames() { 
        static Array<std::string> _axes{upd_axes(0), upd_axes(1), upd_axes(2)};
        return _axes;
    }

...
void Model::createMultibodyTree()
{
    ...
    _multibodyTree.addBody(ground.getAbsolutePathName().toString(),
                           0, false, &ground);

    auto bodies = getComponentList<Body>();
    for (auto& body : bodies) {
        _multibodyTree.addBody( body.getAbsolutePathName().toString(),
                                body.getMass(), false,
    ...
    for (auto& joint : joints) {
        std::string name = joint->getAbsolutePathName().toString();
        ...
        _multibodyTree.addJoint(name,
            ...
            joint->getParentFrame().findBaseFrame().getAbsolutePathName().toString(),
            joint->getChildFrame().findBaseFrame().getAbsolutePathName().toString(),
            false,
            joint.get());
    }
}

...
void Scholz2015GeometryPath::produceForces(const SimTK::State& state,
    ...
    for (const auto& [ix, eltIx] : _obstacleIndexes) {
        ...
        // obstacle body frame B, to conform with the ForceConsumer interface.
        ...
        SimTK::Vec3 p_BP = ground.findStationLocationInAnotherFrame(
                state, p_GP, frame);
        SimTK::Vec3 p_BQ = ground.findStationLocationInAnotherFrame(
                state, p_GQ, frame);

        // Consume the forces applied at the initial and final Frenet frames
        // associated with the obstacle.
        forceConsumer.consumePointForce(state, frame, p_BP, tension * t_P);
        forceConsumer.consumePointForce(state, frame, p_BQ, tension * t_Q);
    }

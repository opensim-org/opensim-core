...
void FreeJoint::extendInitStateFromProperties(SimTK::State& s) const
{
    ...
    if (!matter.getUseEulerAngles(s)){
        ...
        SimTK::Rotation r(SimTK::BodyRotationSequence, xangle, SimTK::XAxis,
                yangle, SimTK::YAxis, zangle, SimTK::ZAxis);
        SimTK::Vec3 t(
                getCoordinate(FreeJoint::Coord::TranslationX).getDefaultValue(),
                getCoordinate(FreeJoint::Coord::TranslationY).getDefaultValue(),
                getCoordinate(FreeJoint::Coord::TranslationZ)
                        .getDefaultValue());

        //FreeJoint* mutableThis = const_cast<FreeJoint*>(this);
        getChildFrame().getMobilizedBody().setQToFitTransform(
                s, SimTK::Transform(r, t));
    }
}
...
void FreeJoint::extendSetPropertiesFromState(const SimTK::State& state)
{
    ...
    if (!matter.getUseEulerAngles(state)) {
        SimTK::Rotation r = getChildFrame()
                                    .getMobilizedBody()
                                    .getMobilizerTransform(state)
                                    .R();
        SimTK::Vec3 t = getChildFrame()
                                .getMobilizedBody()
                                .getMobilizerTransform(state)
                                .p();

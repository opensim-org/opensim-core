                                      ...
                                      bool reverse) : Joint()
{
    ...
    updSocket<PhysicalFrame>("parent_frame").connect(parent);
    updSocket<PhysicalFrame>("child_frame").connect(child);
}
    ...
    bool reverse) : Joint()
{
    ...
    updSocket<PhysicalFrame>("parent_frame").connect(upd_frames(pix));
    updSocket<PhysicalFrame>("child_frame").connect(upd_frames(cix));
}
...
const Coordinate& Joint::getCoordinate() const {
    ...
    OPENSIM_THROW_IF(numCoordinates() > 1,
                     InvalidCall,
                     "Joint has more than one coordinate. Use the "
...
void
Joint::setChildMobilizedBodyIndex(const SimTK::MobilizedBodyIndex index) const
{ 
...
void Joint::extendAddToSystem(SimTK::MultibodySystem& system) const
{
    ...
    if(get_reverse())
        ...
        getSocket<PhysicalFrame>("child_frame").
            getConnectee().addToSystem(system);
    else // otherwise it is the parent frame
        getSocket<PhysicalFrame>("parent_frame").
            getConnectee().addToSystem(system);
}
    ...
    const SimTK::Vector &mobilityForces) const
{
    ...
    if(nm != mobilityForces.size()){
        throw Exception("Joint::calcEquivalentSpatialForce(): input "
                        "mobilityForces does not match model's mobilities");
    }

    const SimTK::MobilizedBodyIndex &mbx =
        getChildFrame().getMobilizedBodyIndex();
    ...
    SimTK::SpatialVec FB_G =
        calcEquivalentSpatialForceForMobilizedBody(s, mbx, mobilityForces);
    SimTK::SpatialVec FBx_G;

    std::set<SimTK::MobilizedBodyIndex>::const_iterator it = mbds.begin();

    //const SimTK::MobilizedBody &G =
    //    getModel().getMatterSubsystem().getGround();
    //const SimTK::MobilizedBody &B =
    //    getModel().getMatterSubsystem().getMobilizedBody(mbx);
    //SimTK::Vec3 r_BG =
    //    B.expressVectorInAnotherBodyFrame(s, B.getOutboardFrame(s).p(), G);

    while(it != mbds.end()){
        FBx_G = calcEquivalentSpatialForceForMobilizedBody(s, *it,
                                                           mobilityForces);
        ...
        //SimTK::Vec3 r_bG = 
        //    b.expressVectorInAnotherBodyFrame(s,
        //                                      b.getOutboardFrame(s).p(), G);
...
/** Joints only produce power when internal constraint forces have components 
along the mobilities of the joint (for example to satisfy prescribed motion). 
In which case the joint power is the constraint forces projected onto the 
mobilities multiplied by the mobilities (internal coordinate velocities). Only 
constraints internal to the joint are accounted for, not external constraints 
that effect joint motion. */
double Joint::calcPower(const SimTK::State &s) const
{
    double power = 0;
    for(int i = 0; i < numCoordinates(); ++i) {
        if (get_coordinates(i).isPrescribed(s)) {
            // get the reaction force for this coordinate prescribed motion
            // constraint
            const SimTK::Constraint &pc =
                _model->updMultibodySystem().updMatterSubsystem()
                    .getConstraint(get_coordinates(i).
                                   _prescribedConstraintIndex);
...
SimTK::SpatialVec
Joint::calcEquivalentSpatialForceForMobilizedBody(const SimTK::State &s, 
    const SimTK::MobilizedBodyIndex mbx,
    const SimTK::Vector &mobilityForces) const
{
    // Get the mobilized body
    const SimTK::MobilizedBody mbd    =
        getModel().getMatterSubsystem().getMobilizedBody(mbx);
    const SimTK::UIndex        ustart = mbd.getFirstUIndex(s);
    const int                  nu     = mbd.getNumU(s);

    const SimTK::MobilizedBody ground =
        getModel().getMatterSubsystem().getGround();

    if (nu == 0) // No mobility forces (weld joint?).
        return SimTK::SpatialVec(SimTK::Vec3(0), SimTK::Vec3(0));

    // Construct the H (joint Jacobian, joint transition) matrix for this
    // mobilizer
    ...
    // To obtain the joint Jacobian, H_PB (H_FM in Simbody) need to be realized
    // to at least position
    _model->getMultibodySystem().realize(s, SimTK::Stage::Position);

    SimTK::Vector f(nu, 0.0);
    for(int i =0; i<nu; ++i){
        f[i] = mobilityForces[ustart + i];
        // Get the H matrix for this Joint by constructing it from the
        // operator H*u
    ...
    // Now solve the pseudoinverse for torque for any unaccounted f:
    // Fw = pinv(~H_PB_G_w)*(f - ~H_PB_G_v*Fv);
    ...
    // Now we have two solution with either the body force Fv or body torque
    // accounting for some or all of f
    ...
    // if body force can contributes at all
    if(fv.norm() > SimTK::SignificantReal){ 
    ...
    // Re-express forces in ground, first by describing force in the
    // parent, Po, frame instead of joint frame
    SimTK::Vec3 vecFw = R_PPo*SimTK::Vec3::getAs(&Fw[0]);
    SimTK::Vec3 vecFv = R_PPo*SimTK::Vec3::getAs(&Fv[0]);

    // Force Acting on joint frame, B,  in child body expressed in Parent
    // body, Po
...
void Joint::updateFromXMLNode(SimTK::Xml::Element& aNode, int versionNumber)
{
    ...
    if (documentVersion < XMLDocument::getLatestVersion()){
        if (documentVersion<30500){
            XMLDocument::renameChildNode(aNode, "location",
                                         "location_in_child"); 
            XMLDocument::renameChildNode(aNode, "orientation",
                                         "orientation_in_child");
        }
        // Version 30501 converted Socket_Body_ to Socket_PhysicalFrame_
        if (documentVersion < 30501) {
            // Handle any models that have the Joint connecting to Bodies
            // instead
            // of PhyscialFrames
            XMLDocument::renameChildNode(aNode, "Socket_Body_",
                                                "Socket_PhysicalFrame_");
        }
        // Version 30505 changed "parent_body" socket name to "parent_frame"
        // Convert location and orientation into PhysicalOffsetFrames owned by
        // the Joint
        if (documentVersion < 30505) {
            ...
            SimTK::Xml::element_iterator sockets_node =
                aNode.element_begin("sockets");

            SimTK::Xml::element_iterator socketElement =
                sockets_node->element_begin("Socket_PhysicalFrame_");
            while (socketElement != aNode.element_end()) {
                // If socket name is "parent_body" rename it to "parent_frame"
                if (socketElement->getRequiredAttributeValue("name") ==
                    "parent_body") {
                    socketElement->setAttributeValue("name", "parent_frame");
                }
                // If socket name is "parent_frame" get the name of the
                // connectee
                if (socketElement->getRequiredAttributeValue("name") ==
                    "parent_frame"){
                    parentNameElt =
                        socketElement->element_begin("connectee_name");
                    parentNameElt->getValueAs<std::string>(parentFrameName);
                }
                if (socketElement->getRequiredAttributeValue("name") ==
                    "child_body") {
                    socketElement->setAttributeValue("name", "child_frame");
                }
                if (socketElement->getRequiredAttributeValue("name") ==
                    "child_frame") {
                    childNameElt =
            ...
            if ((location_in_parent.norm() > 0.0) ||
                ...
                XMLDocument::addPhysicalOffsetFrame(aNode,
    ...
    const int& startingCoordinateIndex) const
{
    ...
    if (mobilized){
        // Index can only be assigned to a parent or child body connected by
        // this Joint
        ...
        SimTK_ASSERT3( ( (mobilized == &getParentFrame()) || 
                       ...
                       "%s::'%s' - Cannot assign underlying system index to a"
                       " PhysicalFrame '%s', which is not a parent or child"
                       " Frame of this Joint.",
        ...
        // Do the check and set it here as well since only the Joint can set
        // the index.
        const PhysicalOffsetFrame* physOff =
            dynamic_cast<const PhysicalOffsetFrame*>(mobilized);
        if (physOff) {
            physOff->getParentFrame().
                setMobilizedBodyIndex(mobod.getMobilizedBodyIndex());
        }
    }
    ...
    for (int iq = 0; iq < numMobilities; ++iq){
        if (j < nc){ // assign
            ...
            std::string msg = getConcreteClassName() +
                " creating MobilizedBody with more mobilities than "

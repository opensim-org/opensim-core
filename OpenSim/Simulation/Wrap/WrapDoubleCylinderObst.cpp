...
void WrapDoubleCylinderObst::setupProperties()
{
    ...
    _radiusUcylProp.setName("radiusUcyl");
    _radiusUcylProp.setValue(-1.0);
    _propertySet.append(&_radiusUcylProp);
    _radiusVcylProp.setName("radiusVcyl");
    _radiusVcylProp.setValue(-1.0);
    _propertySet.append(&_radiusVcylProp);

    _wrapUcylDirectionNameProp.setName("wrapUcylDirection");
    _wrapUcylDirectionNameProp.setValue("Unassigned");
    _propertySet.append(&_wrapUcylDirectionNameProp);
    _wrapVcylDirectionNameProp.setName("wrapVcylDirection");
    _wrapVcylDirectionNameProp.setValue("Unassigned");
    _propertySet.append(&_wrapVcylDirectionNameProp);
    _wrapVcylHomeBodyNameProp.setName("wrapVcylHomeBody");
    _wrapVcylHomeBodyNameProp.setValue("Unassigned");
    _propertySet.append(&_wrapVcylHomeBodyNameProp);
...
void WrapDoubleCylinderObst::connectToModelAndBody(Model& aModel, OpenSim::Body& aBody)
{
    ...
    if (_wrapUcylDirectionName == "righthand" || _wrapUcylDirectionName == "right" || _wrapUcylDirectionName == "righthanded" || _wrapUcylDirectionName == "Righthand" || _wrapUcylDirectionName == "Right" || _wrapUcylDirectionName == "Righthanded")
        _wrapUcylDirection = righthand;
    else if (_wrapUcylDirectionName == "lefthand"  || _wrapUcylDirectionName == "left"  || _wrapUcylDirectionName == "lefthanded"  || _wrapUcylDirectionName == "Lefthand"  || _wrapUcylDirectionName == "Left"  || _wrapUcylDirectionName == "Lefthanded")
        _wrapUcylDirection = lefthand;
    else if (_wrapUcylDirectionName == "Unassigned") // wrapDirection was not specified in obstacle object definition; use default
    {
        _wrapUcylDirection = righthand;
        _wrapUcylDirectionName = "righthand";
    }
    ...
    if (_wrapVcylDirectionName == "righthand" || _wrapVcylDirectionName == "right" || _wrapVcylDirectionName == "righthanded" || _wrapVcylDirectionName == "Righthand" || _wrapVcylDirectionName == "Right" || _wrapVcylDirectionName == "Righthanded")
        _wrapVcylDirection = righthand;
    else if (_wrapVcylDirectionName == "lefthand"  || _wrapVcylDirectionName == "left"  || _wrapVcylDirectionName == "lefthanded"  || _wrapVcylDirectionName == "Lefthand"  || _wrapVcylDirectionName == "Left"  || _wrapVcylDirectionName == "Lefthanded")
        _wrapVcylDirection = lefthand;
    else if (_wrapVcylDirectionName == "Unassigned") // wrapDirection was not specified in obstacle object definition; use default
    {
        _wrapVcylDirection = righthand;
        _wrapVcylDirectionName = "righthand";
    }
                                     ...
                                     const PathWrap& aPathWrap, WrapResult& aWrapResult, bool& aFlag) const
{
    ...
    U[2]=_translation[2];
    double V[3];
    V[0]=_translationVcyl[0];
    V[1]=_translationVcyl[1];
    V[2]=_translationVcyl[2];
    ...
    double P[3],q[3],Q[3],Pq,qQ,QT,ru;
    P[0]=aPoint1[0];
    P[1]=aPoint1[1];
    P[2]=aPoint1[2];
    double S[3],t[3],T[3],Tt,tS,L,rv;
    S[0]=aPoint2[0];
    S[1]=aPoint2[1];
    S[2]=aPoint2[2];
    ...
    double UcylObstToUcylBody[9];
    load_Rxyz(xyzBodyRotation,UcylObstToUcylBody);
    double VcylObstToVcylBody[9];
    load_Rxyz(xyzBodyRotationVcyl,VcylObstToVcylBody);
    double UcylBodyToGround[9];
    _model->getSimbodyEngine().getDirectionCosines(s, *_wrapUcylHomeBody,UcylBodyToGround);
    double VcylBodyToGround[9];
    _model->getSimbodyEngine().getDirectionCosines(s, *_wrapVcylHomeBody,VcylBodyToGround);
    double VcylBodyToUcylBody[9];
    quick_mul_mtxT_by_mtx(UcylBodyToGround,VcylBodyToGround,VcylBodyToUcylBody);
    double VcylObstToUcylBody[9];
    quick_mul_mtx_by_mtx(VcylBodyToUcylBody,VcylObstToVcylBody,VcylObstToUcylBody);
    ...
    if( (int)aWrapResult.c1[1] != 12345 ) {
        aWrapResult.c1[1]=12345.01;    // Initialize activeState first iteration
        activeState=0;
    }
...
//	_activeState=activeState;		???? I CAN'T SEEM TO SET THIS!!!  GARNER - Needs to be Set in a WrapResult variable.
    ...
    aWrapResult.r1[0]=q[0];
    aWrapResult.r1[1]=q[1];
    aWrapResult.r1[2]=q[2];
    aWrapResult.r2[0]=t[0];
    aWrapResult.r2[1]=t[1];
    aWrapResult.r2[2]=t[2];
...
getVcylToUcylRotationMatrix(const SimTK::State& s, double VcylObstToUcylObst[9]) const {
    double xyzBodyRotation[3] = { _xyzBodyRotation[0], _xyzBodyRotation[1], _xyzBodyRotation[2] };
    double xyzBodyRotationVcyl[3] = { _xyzBodyRotationVcyl[0],_xyzBodyRotationVcyl[1],_xyzBodyRotationVcyl[2] };
    double UcylBodyToUcylObst[9];
    load_Rxyz(xyzBodyRotation,UcylBodyToUcylObst);
    double VcylBodyToVcylObst[9];
    load_Rxyz(xyzBodyRotationVcyl,VcylBodyToVcylObst);
    double UcylBodyToGround[9];
    _model->getSimbodyEngine().getDirectionCosines(s, *_wrapUcylHomeBody,UcylBodyToGround);
    double VcylBodyToGround[9];
    _model->getSimbodyEngine().getDirectionCosines(s, *_wrapVcylHomeBody,VcylBodyToGround);
    ...
    quick_mul_mtxT_by_mtx(UcylBodyToGround,VcylBodyToGround,VcylBodyToUcylBody);
    double VcylObstToUcylBody[9];
    quick_mul_mtx_by_mtxT(VcylBodyToUcylBody,VcylBodyToVcylObst,VcylObstToUcylBody);
...
static int single_cylinder(double R,double P[3],double q[3],double t[3],double S[3],
    ...
    double dp,ds,rtp,rts,sgn,Rpos,buff;
    *ru=R;
    sgn=SIGN(R);
    Rpos=ABS(R);
    buff=(Rpos+OBST_BUFF)*(Rpos+OBST_BUFF);
    /*==================================================================*/
    dp=P[0]*P[0]+P[1]*P[1];
    rtp=dp-R*R;
    ds=S[0]*S[0]+S[1]*S[1];
    ...
    if((dp<buff)||(ds<buff)) {	/* ONE POINT IS INSIDE CYLINDER SO SHRINK IT */
        if(dp<ds)	{
            R=sgn*(sqrt(dp)-OBST_BUFF);
            rtp=dp-R*R;
            rts=ds-R*R;
        }
        else        {
            R=sgn*(sqrt(ds)-OBST_BUFF);
            rtp=dp-R*R;
            rts=ds-R*R;
        }
    }	*ru=R;
    ...
    dp=R/dp;
    rtp=sqrt(rtp);
    q[0]=(R*P[0]-rtp*P[1])*dp;
    q[1]=(R*P[1]+rtp*P[0])*dp;
    ds=R/ds;
    rts=sqrt(rts);
    t[0]=(R*S[0]+rts*S[1])*ds;
    t[1]=(R*S[1]-rts*S[0])*ds;
    /*==================================================================*/
...
static int double_cylinder(double U[3],double Ru,double V[3],double Rv,double M[9],
    ...
    *rv=Rv;
    ...
    if(*active!=3) switch(*active) {
        ...
        case 0:	/* NEITHER CYLINDER IS ACTIVE */
            ...
            t[1]=S[1];
            t[2]=S[2];
            quick_mul_vec_by_mtx(S,M,C);
            quick_add_vec_to_vec(C,V,C);
            *Pq=0.0;
            *qQ=0.0;
            *L=*QT=DISTANCE_BETWEEN(P,C);
            *Tt=0.0;
            *tS=0.0;
            if(path_inside_radius(P,C,Ru)) {
                *active=1;
                RETURN_DOUBLE;
            }
            quick_mul_vec_by_mtxT(P,M,C);
            quick_add_vec_to_vec(C,U,C);
            if(path_inside_radius(C,S,Rv)) {
                *active=2;
                RETURN_DOUBLE;
            }
            return(0);
        ...
        case 1:	/* U CYLINDER IS ACTIVE ALONE */
            ...
            *tS=0.0;
            i=1;
            single_cylinder(Ru,P,q,Q,C,Pq,qQ,QT,L,&i,ru);
            if(!i) {
                Q[0]=P[0];
                Q[1]=P[1];
                Q[2]=P[2];	/* NIETHER SHOULD BE ACTIVE */
                *QT=*qQ;
                *qQ=0.0;
                *active=0;
                quick_mul_vec_by_mtxT(P,M,C);
                quick_add_vec_to_vec(C,U,C);	/*=2?*/
                if(path_inside_radius(C,S,Rv)) {
                    *active=2;
                    RETURN_DOUBLE;
                }
        ...
        case 2:	/* V CYLINDER IS ACTIVE ALONE */
            ...
            *qQ=0.0;
            i=1;
            single_cylinder(Rv,C,T,t,S,QT,Tt,tS,L,&i,rv);
            if(!i) {
                T[0]=S[0];
                T[1]=S[1];
                T[2]=S[2]; /* NIETHER SHOULD BE ACTIVE */
                *QT=*Tt;
                *Tt=0.0;
                *active=0;
                return(0);
            }
            quick_mul_vec_by_mtx(T,M,C);
            quick_add_vec_to_vec(C,V,C);
            if(path_inside_radius(P,C,Ru)) {
                *active=3;
                RETURN_DOUBLE;
    ...
    *rv=Rv;
    dp=Ru/dp;
    rtp=sqrt(rtp);
    q[0]=(Ru*P[0]-rtp*P[1])*dp;
    q[1]=(Ru*P[1]+rtp*P[0])*dp;
    ds=Rv/ds;
    rts=sqrt(rts);
    t[0]=(Rv*S[0]+rts*S[1])*ds;
    t[1]=(Rv*S[1]-rts*S[0])*ds;
    *Pq=sqrt((q[0]-P[0])*(q[0]-P[0])+(q[1]-P[1])*(q[1]-P[1]));
    *tS=sqrt((t[0]-S[0])*(t[0]-S[0])+(t[1]-S[1])*(t[1]-S[1]));
    /*==================================================================*/
    ...
    for(count=0; count<MAXCOUNT; count++) {
        ...
        /*===============================================================*/
        quick_mul_vec_by_mtx(x,M,TnU);
        quick_add_vec_to_vec(TnU,V,TnU);
        /*===============================================================*/
        Td=TnU[0]*TnU[0]+TnU[1]*TnU[1];
        Trt=Td-Ru*Ru;
        Td=Ru/Td;
        if(Trt<0.0) return(-1);
        Trt=sqrt(Trt);
        Q[0]=(Ru*TnU[0]+Trt*TnU[1])*Td;
        Q[1]=(Ru*TnU[1]-Trt*TnU[0])*Td;
        /*===============================================================*/
        *QT=sqrt((TnU[0]-Q[0])*(TnU[0]-Q[0])+(TnU[1]-Q[1])*(TnU[1]-Q[1]));
        Tc=1.0-0.5*((q[0]-Q[0])*(q[0]-Q[0])+(q[1]-Q[1])*(q[1]-Q[1]))/(Ru*Ru);
        if(fabs(Tc)>=1.0) *qQ=fabs(Ru*SimTK::Pi);
        else	*qQ=fabs(Ru*acos(Tc));
        /*===============================================================*/
        LU=1.0/(*Pq+*QT+*qQ);
        q[2]=P[2]+(TnU[2]-P[2])*(*Pq)*LU;
        Q[2]=TnU[2]+(P[2]-TnU[2])*(*QT)*LU;
        /*===============================================================*/
        quick_mul_vec_by_mtxT(Q,M,QnV);
        quick_add_vec_to_vec(QnV,U,QnV);
        /*===============================================================*/
        Qd=QnV[0]*QnV[0]+QnV[1]*QnV[1];
        Qrt=Qd-Rv*Rv;
        Qd=Rv/Qd;
        if(Qrt<0.0) return(-1);
        Qrt=sqrt(Qrt);
        T[0]=(Rv*QnV[0]-Qrt*QnV[1])*Qd;
        T[1]=(Rv*QnV[1]+Qrt*QnV[0])*Qd;
        /*===============================================================*/
        TQ=sqrt((T[0]-QnV[0])*(T[0]-QnV[0])+(T[1]-QnV[1])*(T[1]-QnV[1]));
        Qc=1.0-0.5*((t[0]-T[0])*(t[0]-T[0])+(t[1]-T[1])*(t[1]-T[1]))/(Rv*Rv);
        if(fabs(Qc)>=1.0) *Tt=fabs(Rv*SimTK::Pi);
        else	*Tt=fabs(Rv*acos(Qc));
        /*===============================================================*/
        LV=1.0/(*tS+TQ+*Tt);
        ...
        dQdx[1]=  (Trt+TnU[1]*TnU[1]*rtp-2*TnU[1]*Q[0]*dp)*Td;
        dQdx[2]=0.0;
        dQdx[3]= -(Trt+TnU[0]*TnU[0]*rtp+2*TnU[0]*Q[1]*dp)*Td;
        dQdx[4]=  (Ru -TnU[0]*TnU[1]*rtp-2*TnU[1]*Q[1]*dp)*Td;
        dQdx[5]=0.0;
        z1= *QT*(P[2]-TnU[2])*LU*LU;
        z2=(P[2]-TnU[2])*LU-z1;
        ...
        dTdQ[1]= -(Qrt+QnV[1]*QnV[1]*rtp+2*QnV[1]*T[0]*dp)*Qd;
        dTdQ[2]=0.0;
        dTdQ[3]=  (Qrt+QnV[0]*QnV[0]*rtp-2*QnV[0]*T[1]*dp)*Qd;
        dTdQ[4]=  (Rv +QnV[0]*QnV[1]*rtp-2*QnV[1]*T[1]*dp)*Qd;
        dTdQ[5]=0.0;
        z1=TQ*(S[2]-QnV[2])*LV*LV;
        z2=(S[2]-QnV[2])*LV-z1;
        ...
        dTdQ[8]=  1-TQ*LV;
        /*===============================================================*/
        quick_mul_mtx_by_mtx(dTdQ,dQdx,Cx);
        Cx[0]-=1.0;
        Cx[4]-=1.0;
        Cx[8]-=1.0;
        quick_solve_linear(3,Cx,C,C);
        quick_sub_vec_fm_vec(x,C,x);	/* UPDATE x */
        /*===============================================================*/
    }
    T[0]=x[0];
    T[1]=x[1];
    T[2]=x[2];	/*== APPLY SOLUTION ==*/
    if(count>=MAXCOUNT) {
    ...
    if(Ru*(Q[0]*q[1]-Q[1]*q[0])>0.0) {	/* U CYLINDER SHOULD BE INACTIVE */
        ...
        *qQ=0.0;
        if(!i) {
            *active=0;    /* NIETHER CYLINDER SHOULD BE ACTIVE */
            return(0);
        }
    } else if(Rv*(t[0]*T[1]-t[1]*T[0])>0.0) {	/* V CYLINDER SHOULD BE INACTIVE */
        t[0]=S[0];
        t[1]=S[1];
        t[2]=S[2];
        T[0]=S[0];
        ...
        *active=3;						/* BOTH CYLINDERS SHOULD REMAIN ACTIVE */
        *Pq=sqrt((*Pq)*(*Pq)+(q[2]-P[2])*(q[2]-P[2]));

...
void testAccessByTime() {
    ...
    // Nearest before.
    SimTK_TEST(states.findIndexNearestBefore(0.5) == 0);
    SimTK_TEST(states.findIndexNearestBefore(0.5000001) == 0);
    SimTK_TEST(states.findIndexNearestBefore(1.299999999) == 0);

    SimTK_TEST(states.findIndexNearestBefore(1.3) == 1);
    SimTK_TEST(states.findIndexNearestBefore(1.300001) == 1);
    SimTK_TEST(states.findIndexNearestBefore(3.49999) == 1);

    SimTK_TEST(states.findIndexNearestBefore(4.1) == 3);
    SimTK_TEST(states.findIndexNearestBefore(4.2) == 3);
    SimTK_TEST(states.findIndexNearestBefore(153.7) == 3);

    // Nearest after.
    SimTK_TEST(states.findIndexNearestAfter(-100.0) == 0);
    SimTK_TEST(states.findIndexNearestAfter(0) == 0);
    SimTK_TEST(states.findIndexNearestAfter(0.1) == 0);
    SimTK_TEST(states.findIndexNearestAfter(0.4999999999) == 0);
    SimTK_TEST(states.findIndexNearestAfter(0.5) == 0);

    SimTK_TEST(states.findIndexNearestAfter(1.3000000000001) == 2);
    SimTK_TEST(states.findIndexNearestAfter(1.31) == 2);
    SimTK_TEST(states.findIndexNearestAfter(1.4) == 2);
    SimTK_TEST(states.findIndexNearestAfter(3.4) == 2);
    SimTK_TEST(states.findIndexNearestAfter(3.4999999999999) == 2);
    SimTK_TEST(states.findIndexNearestAfter(3.5) == 2);
    
    SimTK_TEST(states.findIndexNearestAfter(4.0001) == 3);
    SimTK_TEST(states.findIndexNearestAfter(4.1) == 3);

    // Exception if there is not a state before/after the specified time.
    SimTK_TEST_MUST_THROW_EXC(states.findIndexNearestBefore(0.25),
            StatesTrajectory::TimeOutOfRange);
    SimTK_TEST_MUST_THROW_EXC(states.findIndexNearestBefore(0.0),
            StatesTrajectory::TimeOutOfRange);
    SimTK_TEST_MUST_THROW_EXC(states.findIndexNearestBefore(-1.0),
            StatesTrajectory::TimeOutOfRange);
    SimTK_TEST_MUST_THROW_EXC(states.findIndexNearestBefore(.499999),
            StatesTrajectory::TimeOutOfRange);

    SimTK_TEST_MUST_THROW_EXC(states.findIndexNearestAfter(4.10000001),
            StatesTrajectory::TimeOutOfRange);
    SimTK_TEST_MUST_THROW_EXC(states.findIndexNearestAfter(1050.9),
            StatesTrajectory::TimeOutOfRange);
    ...
    // Simulate the scenario that the tolerance is intended to handle.
    {
        ...
        // the tolerance is 0:
        SimTK_TEST(states2.findIndexNearestAfter(4.0) != 1);
        // The user would *expect* to get index 3, but does not, since 
        // the tolerance is 0:
        SimTK_TEST(states2.findIndexNearestBefore(5.0) != 3);
    ...
    SimTK_TEST(states.findIndexNearestBefore(0.499, 0.001) == 0);
    SimTK_TEST(states.findIndexNearestBefore(0.495, 0.005) == 0);
    SimTK_TEST(states.findIndexNearestBefore(1.2991, 0.001) == 1);
    SimTK_TEST(states.findIndexNearestBefore(1.298999, 0.001) == 0);
    SimTK_TEST(states.findIndexNearestBefore(4.099, 0.001) == 3);
    SimTK_TEST(states.findIndexNearestBefore(4.098, 0.001) == 2);

    SimTK_TEST(states.findIndexNearestAfter(0.501, 0.001) == 0);
    SimTK_TEST(states.findIndexNearestAfter(0.503, 0.003) == 0);
    // Past the tolerance, we get the next state.
    SimTK_TEST(states.findIndexNearestAfter(0.50301, 0.003) == 1);
    SimTK_TEST(states.findIndexNearestAfter(1.301, 0.001) == 1);
    SimTK_TEST(states.findIndexNearestAfter(1.301001, 0.001) == 2);
    SimTK_TEST(states.findIndexNearestAfter(4.1005, 0.001) == 3);
    SimTK_TEST(states.findIndexNearestAfter(4.101, 0.001) == 3);

    // Test exceptions with custom tolerance.
    SimTK_TEST_MUST_THROW_EXC(states.findIndexNearestBefore(0.498, 0.001),
            StatesTrajectory::TimeOutOfRange);
    SimTK_TEST_MUST_THROW_EXC(states.findIndexNearestBefore(0.498999, 0.001),
            StatesTrajectory::TimeOutOfRange);
    SimTK_TEST_MUST_THROW_EXC(states.findIndexNearestAfter(4.102, 0.001),
            StatesTrajectory::TimeOutOfRange);
    SimTK_TEST_MUST_THROW_EXC(states.findIndexNearestAfter(4.10100001, 0.001),
            StatesTrajectory::TimeOutOfRange);
    ...
    SimTK_TEST(states.findIndexNearestAfter(4.0) == 3);
    SimTK_TEST(states.findIndexNearestAfter(4.1) == 3);
    SimTK_TEST(states.findIndexNearestBefore(5.0) == 5);
    SimTK_TEST(states.findIndexNearestBefore(4.1) == 5);

    SimTK_TEST(states.findIndexNearestAfter(4.1004999, 0.0005) == 3);
    SimTK_TEST(states.findIndexNearestBefore(4.0995, 0.0005) == 5);

    SimTK_TEST(states.findIndexNearestAfter(5.8) == 6);
    SimTK_TEST(states.findIndexNearestBefore(5.8) == 7);
    SimTK_TEST_MUST_THROW_EXC(states.findIndexNearestAfter(5.800001),
            StatesTrajectory::TimeOutOfRange);
    ...
    // the tolerance.
    {
        ...
        // is 0.01. If they ask for the nearest state before 3.50, they should
        // get the state that is actually at 3.502, since, from the
        // console/file, this appears to be the first state "before" 3.50.
        StatesTrajectory states2;
        state.setTime(0.50);
        states2.append(state); // 0
        state.setTime(1.30);
        ...
        SimTK_TEST(states2.findIndexNearestBefore(3.499, 0.010) == 4);
        ...
        SimTK_TEST(states3.findIndexNearestAfter(3.5, 0.010) == 2);
    }
    ...
    // Iterate across the states between two times.
    {
        ...
        int index = 0;
        for (const auto& state : states.findBetween(1.0, 4.0)) {
    ...
    {
        int count = 0;
        for (const auto& state : states.findBetween(2.0, 2.1)) {
            count++;
        }
        SimTK_TEST(count == 0);
    }
    ...
    {
        int count = 0;
        for (const auto& state : states.findBetween(4.1, 4.1)) {
            count++;
        }
        SimTK_TEST(count == 3);
    }
    // Test iterating with a tolerance.
    {
        ...
        int index = 0;
        for (const auto& state : states.findBetween(1.301, 4.099, 0.01)) {
            SimTK_TEST(&state == &states.get(indices.at(index)));
            index++;
        }
    }
    ...
    {
        int count = 0;
        for (const auto& state : states.findBetween(1000.0, 1010.0)) {
            count++;
        }
        SimTK_TEST(count == 0);
    }
    {
        int count = 0;
        for (const auto& state : states.findBetween(-50, -40.3)) {
            count++;
        }
        SimTK_TEST(count == 0);
    }
    ...
    {
        SimTK_TEST_MUST_THROW_EXC(states.findBetween(4.5, 1.1),
                SimTK::Exception::APIArgcheckFailed);
    }
    // Test trying to use endTime > startTime beyond the valid times.
    {
        SimTK_TEST_MUST_THROW_EXC(states.findBetween(2000.0, 1580.0),
                SimTK::Exception::APIArgcheckFailed);
    }
    // Test on an empty trajectory.
    {
        ...
        for (const auto& state : emptyTraj.findBetween(1.6, 6.2)) {
            count++;
        }
        SimTK_TEST(count == 0);
    }
...
void testAccessAtTime() {
...

    {
        ...
        SimTK_TEST(states.findAt(4.0) == states.end());
        // Trying to get the index gives an exception.
        SimTK_TEST_MUST_THROW_EXC(states.findIndexAt(3.0), OpenSim::Exception);
    }
    {
        // Make a trajectory with different times.
        StatesTrajectory states;
        ...
        SimTK_TEST(states.findIndexAt(0.5) == 0);
        SimTK_TEST(states.findIndexAt(1.3) == 1);
        SimTK_TEST(states.findIndexAt(3.5) == 2);
        SimTK_TEST(states.findIndexAt(4.1) == 3);

        // Asking for a state at a time that does not exist gives an exception.
        SimTK_TEST_MUST_THROW_EXC(states.findIndexAt(-10.0),
                                  StatesTrajectory::NoStateAtTime);
        SimTK_TEST_MUST_THROW_EXC(states.findIndexAt(0.25),
                                  StatesTrajectory::NoStateAtTime);
        SimTK_TEST_MUST_THROW_EXC(states.findIndexAt(0.5 - SignificantReal),
                                  StatesTrajectory::NoStateAtTime);
        SimTK_TEST_MUST_THROW_EXC(states.findIndexAt(0.5 + SignificantReal),
                                  StatesTrajectory::NoStateAtTime);
        SimTK_TEST_MUST_THROW_EXC(states.findIndexAt(2.0),
                                  StatesTrajectory::NoStateAtTime);
        SimTK_TEST_MUST_THROW_EXC(states.findIndexAt(4.0),
                                  StatesTrajectory::NoStateAtTime);
        SimTK_TEST_MUST_THROW_EXC(states.findIndexAt(4.1 + SignificantReal),
                                  StatesTrajectory::NoStateAtTime);
        SimTK_TEST_MUST_THROW_EXC(states.findIndexAt(4.10000001),
                                  StatesTrajectory::NoStateAtTime);
        SimTK_TEST_MUST_THROW_EXC(states.findIndexAt(5.5),
                                  StatesTrajectory::NoStateAtTime);

        // With a tolerance.
        SimTK_TEST(states.findIndexAt(0.50001, 1.0e-5) == 0);
        SimTK_TEST(states.findIndexAt(0.49999, 1.0e-5 + SignificantReal) == 0);
        SimTK_TEST(states.findIndexAt(1.299999, 1.0e-6 + SignificantReal) == 1);
        SimTK_TEST(states.findIndexAt(1.300001, 1e-6) == 1);
        SimTK_TEST(states.findIndexAt(3.499, 1e-3) == 2);
        SimTK_TEST(states.findIndexAt(3.501, 1e-3) == 2);
        SimTK_TEST(states.findIndexAt(4.09999999, 1e-8) == 3);
        SimTK_TEST(states.findIndexAt(4.10000001, 1e-8) == 3);

        // Asking for a state that doesn't exist, even with the tolerance.
        SimTK_TEST_MUST_THROW_EXC(
                states.findIndexAt(3.6 + SimTK::SignificantReal, 0.1),
                StatesTrajectory::NoStateAtTime);
        SimTK_TEST_MUST_THROW_EXC(
                states.findIndexAt(4.12 + SimTK::SignificantReal, 0.02),
                StatesTrajectory::NoStateAtTime);
    }
    ...
    // the tolerance.
    {
        ...
        SimTK_TEST(states.findIndexAt(8.044, 0.01) == 1);
        SimTK_TEST(states.findIndexAt(8.046, 0.01) == 2);

        SimTK_TEST(states.findIndexAt(8.04499999999, 0.01) == 1);
        SimTK_TEST(states.findIndexAt(8.04500000001, 0.01) == 2);
        ...
        SimTK_TEST(states.findIndexAt(8.044, 5.0) == 1);
        SimTK_TEST(states.findIndexAt(8.046, 5.0) == 2);
    }
    ...
    // the tolerance, even at the same time.
    {
        ...
        SimTK_TEST(states.findIndexAt(7.5) == 1);
        SimTK_TEST(states.findIndexAt(7.9) == 4);
        SimTK_TEST(states.findIndexAt(7.4, 0.1) == 1);
        SimTK_TEST(states.findIndexAt(7.56, 0.06) == 3);
        SimTK_TEST(states.findIndexAt(7.56, 0.10) == 3);
        SimTK_TEST(states.findIndexAt(7.88, 0.03) == 4);
        SimTK_TEST(states.findIndexAt(7.93, 0.05) == 5);
    }
}

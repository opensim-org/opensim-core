...
namespace {
    ...
    //==========================================================================
    double computeMomentArmFromDefinition(const SimTK::State &s,
    ...
    SimTK::Vector computeGenForceScaling(const Model &osimModel,
                                         const SimTK::State &s,
                                         const Coordinate &coord,
    ...
    //==========================================================================
    // Main test driver can be used on any model so test cases should be very
    // easy to add
    //==========================================================================
            ...
            double mass = -1.0, string errorMessage = "")
    {
        ...
        Coordinate &coord = (coordName != "") ?
                            osimModel.updCoordinateSet().get(coordName) :
                            osimModel.updCoordinateSet()[0];

        // Consider one force, which is the muscle of interest
        Muscle &muscle = (muscleName != "") ?
            dynamic_cast<Muscle&>((osimModel.updMuscles().get(muscleName))) :
        ...
        for(int i=0; i<osimModel.getConstraintSet().getSize(); i++){
            ...
            if(aConstraint.getConcreteClassName() == "CoordinateCouplerConstraint"){
                CoordinateCouplerConstraint& coupler =
                        dynamic_cast<CoordinateCouplerConstraint&>(aConstraint);
        ...
        for(int i = 0; i <=nsteps; i++){
            ...
            if (mass!=0 ) {
                ...
                osimModel.getMultibodySystem().getMatterSubsystem()
                        .calcTreeEquivalentMobilityForces(s, appliedBodyForces,
                                                          equivalentGenForce);

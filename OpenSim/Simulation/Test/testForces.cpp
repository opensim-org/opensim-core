...
/*
This test inspects the contact between 
*/
void testSmith2018ArticularContactForce() {
    ...
    Body* femur = new Body("femur", 0.5, SimTK::Vec3(0), 
        SimTK::Inertia::brick(0.2, 0.2, 0.2));
    model.addBody(femur);

    WeldJoint* weld = new WeldJoint("weld", model.getGround(), *femur);
    model.addJoint(weld);

    Body* tibia = new Body("tibia", 0.5, SimTK::Vec3(0), 
        SimTK::Inertia::brick(0.2, 0.2, 0.2));
    model.addBody(tibia);

    SliderJoint* slider = new SliderJoint("slider", *femur, *tibia);
    slider->updCoordinate().setName("height");
    model.addJoint(slider);

    Smith2018ContactMesh* femur_mesh = new Smith2018ContactMesh("femur_mesh",
        "lenhart2015-R-femur-cartilage.stl", *femur);
    model.addComponent(femur_mesh);

    Smith2018ContactMesh* tibia_mesh = new Smith2018ContactMesh("tibia_mesh",
        "lenhart2015-R-tibia-cartilage.stl", *tibia);
    model.addComponent(tibia_mesh);

    Smith2018ArticularContactForce* tf_contact =
        new Smith2018ArticularContactForce("tf_contact",
            *femur_mesh, *tibia_mesh, 1000000, 0.5, 0.05);

    model.addComponent(tf_contact);
    ...
    // end timing
    cout << "Smith2018ArticularContactForce simulation time = " 
    ...
    manager.getStateStorage().print(
        "testSmith2018ArticularContactForce_states.sto");
        
    // Save the forces
    reporter->getForceStorage().print(
        "testSmith2018ArticularContactForce_forces.mot"); 

...
void testExponentialContact() {
    ...
    // Load the model
    Model model{"BouncingBlock_ExponentialContact.osim"};
    ...
    const OpenSim::Body& block = model.getBodySet().get("blockEC");
    ...
    cout << "Exponential Contact simulation time = "
         << 1.e3 * (stopTime - startTime) / CLOCKS_PER_SEC << "ms" << endl;

    // Print out the states and contact information
    manager.getStateStorage().print("bouncing_block_EC_states.sto");
    reporter->getForceStorage().print("bouncing_block_EC_forces.sto");

/*
    // Bouncing block should have settled on ground due to dissipation.
    // The force generated by contact then should be identically body
    // weight vertically and zero horizontally.
    OpenSim::ExponentialContact& spr =
            (OpenSim::ExponentialContact&)osimModel.getForceSet().get("Exp0");

    Array<double> contact_force = spr.getRecordValues(state);
    ASSERT_EQUAL(contact_force[0], 0.0, 1e-4); // no horizontal force
    // vertical should be the weight
    ASSERT_EQUAL(contact_force[1], -block.getMass()*gravity_vec[1], 1e-3);
    ASSERT_EQUAL(contact_force[2], 0.0, 1e-4); // no horizontal force
    ASSERT_EQUAL(contact_force[3], 0.0, 1e-4); // no torque on the ball
    ASSERT_EQUAL(contact_force[4], 0.0, 1e-4); // no torque on the ball
    ASSERT_EQUAL(contact_force[5], 0.0, 1e-4); // no torque on the ball
    ...
    if (!isEqual) {
        spr.print("originalForce.xml");
        sprCopy->print("copyOfForce.xml");
    }

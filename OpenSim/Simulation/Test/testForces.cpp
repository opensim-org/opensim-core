...
namespace {
    ...
    void testTranslationalDampingEffect(Model& osimModel, Coordinate& sliderCoord,
        ...
        aPropVec3.setValue(SimTK::Vec3(100.));
...
TEST_CASE("testExpressionBasedCoordinateForce") {
    ...
    ;
    OpenSim::Body ball(
            "ball", mass, SimTK::Vec3(0), mass * SimTK::Inertia::sphere(0.1));
    ball.attachGeometry(new Sphere(0.1));
    ball.scale(SimTK::Vec3(ball_radius), false);

    // Add joints
    SliderJoint slider("slider", ground, SimTK::Vec3(0), SimTK::Vec3(0, 0, SimTK::Pi / 2), ball,
            SimTK::Vec3(0), SimTK::Vec3(0, 0, SimTK::Pi / 2));
    ...
    for (int i = 1; i <= nsteps; i++) {
        ...
        Vec3 pos = ball.findStationLocationInGround(osim_state, SimTK::Vec3(0));
...
TEST_CASE("testExpressionBasedPointToPointForce") {
    ...
    SimTK::Vec3 p1(rand.getValue(), rand.getValue(), rand.getValue());
    SimTK::Vec3 p2(rand.getValue(), rand.getValue(), rand.getValue());
    ...
    OpenSim::Body ball(
            "ball", mass, SimTK::Vec3(0), mass * SimTK::Inertia::sphere(ball_radius));
    ball.attachGeometry(new Sphere(ball_radius));
    ball.scale(SimTK::Vec3(ball_radius), false);

    // define body's joint
    FreeJoint free("free", ground, SimTK::Vec3(0), SimTK::Vec3(0, 0, SimTK::Pi / 2), ball, SimTK::Vec3(0),
            SimTK::Vec3(0, 0, SimTK::Pi / 2));
...
TEST_CASE("testPathSpring") {
    ...
    const Ground& ground = osimModel.getGround();
    ;
    OpenSim::Body pulleyBody("PulleyBody", mass, SimTK::Vec3(0),
            mass * SimTK::Inertia::brick(0.1, 0.1, 0.1));
    OpenSim::Body block("block", mass, SimTK::Vec3(0),
    ...
    WeldJoint weld("pulley", ground, Vec3(0, 1.0, 0), Vec3(0), pulleyBody,
            Vec3(0), Vec3(0));
    SliderJoint slider("slider", ground, Vec3(0), Vec3(0, 0, SimTK::Pi / 2), block,
...
TEST_CASE("testSpringMass") {
    ...
    // Add joints
    SliderJoint slider("slider", ground, Vec3(0), Vec3(0, 0, SimTK::Pi / 2), ball,
            Vec3(0), Vec3(0, 0, SimTK::Pi / 2));
...
TEST_CASE("testBushingForce") {
    ...
    auto* slider = new SliderJoint("slider", ground, Vec3(0),
            Vec3(0, 0, SimTK::Pi / 2), *ball, Vec3(0), Vec3(0, 0, SimTK::Pi / 2));
...
TEST_CASE("testTwoFrameLinkerUpdateFromXMLNode") {
    ...
    auto* spring = new BushingForce("bushing", ground,
            SimTK::Transform(SimTK::Rotation(SimTK::BodyRotationSequence, -0.5, SimTK::XAxis, 0, SimTK::YAxis, 0.5,
                              SimTK::ZAxis),
                    Vec3(1, 2, 3)),
            *ball,
            SimTK::Transform(SimTK::Rotation(SimTK::BodyRotationSequence, 0.1, SimTK::XAxis, 0.2, SimTK::YAxis,
                              0.3, SimTK::ZAxis),
                    Vec3(4, 5, 6)),
...
TEST_CASE("testExpressionBasedBushingForceTranslational") {
    ...
    SliderJoint sliderY("slider", ground, Vec3(0), Vec3(0, 0, SimTK::Pi / 2), ball,
            Vec3(0), Vec3(0, 0, SimTK::Pi / 2));
...
TEST_CASE("testExpressionBasedBushingForceRotational") {
    ...
    PinJoint pin("pin", ground, Vec3(0), Vec3(SimTK::Pi / 2, 0, 0), ball, Vec3(0),
            Vec3(SimTK::Pi / 2, 0, 0));
...
TEST_CASE("testBlankevoort1991Ligament") {
    ...
    SliderJoint* slider = new SliderJoint("slider", ground, Vec3(0.2, 0, 0),
            Vec3(0, 0, SimTK::Pi / 2), *block, Vec3(0), Vec3(0, 0, SimTK::Pi / 2));

...
//      4. HuntCrossleyForce
//      5. ExponentialContact
//      6. SmoothSphereHalfSpaceForce
//      7. CoordinateLimitForce
//      8. RotationalCoordinateLimitForce
//      9. ExternalForce
//     10. PathSpring
...
int main() {
    ...
    try {
        testExponentialContact();
    } catch (const std::exception& e) {
        cout << e.what() << endl;
        failures.push_back("testExponentialContact");
    }
...
void testElasticFoundation() {
    ...
    // Save the forces
    reporter->getForceStorage().print("elastic_contact_forces.sto");
...
void testExponentialContact() {
    ...
    // Start temporary model scope
    {
        ...
        // Joint
        FreeJoint* freeJoint = new FreeJoint("free", model->updGround(),
                Vec3(0), Vec3(0), *block, Vec3(0), Vec3(0));
        model->addBody(block);
        model->addJoint(freeJoint);

        // ExponentialContact Instances----------------------------------
        // Customizable Parameters
        // Default parameters work well, but may be altered.
        // Default values are used here.
        SimTK::ExponentialSpringParameters params;
        // Contact plane transform.
        // Need to make the normal of the contact plane, which is along
        // along the z-axis, point along the y-axis of the Ground frame,
        // which is up in OpenSim.
    ...
    const OpenSim::Body& block = model.getBodySet().get("block");
    const SimTK::MobilizedBody& body = block.getMobilizedBody();
    SimTK::Vec3 pos(0.0);
    pos[1] = 0.5;  // starts from 0.5 above the ground
    ...
    Manager manager(model);
    manager.setIntegratorAccuracy(1e-6);
    manager.setIntegratorMaximumStepSize(0.02);
    state.setTime(0.0);
    manager.initialize(state);
    clock_t startTime = clock();
    state = manager.integrate(10.0);
    clock_t stopTime = clock();
    cout << "Exponential Contact simulation execution time = "
         << 1.e3 * (stopTime - startTime) / CLOCKS_PER_SEC << "ms" << endl;

    // Print out the states and contact information
    manager.getStateStorage().print("BouncingBlock_EC_states.sto");
    reporter->getForceStorage().print("BouncingBlock_EC_forces.sto");

    // Realize to Stage::Dynamics
    // This is necessary to compute elastic anchor points and
    // all of the contact forces.
    model.getMultibodySystem().realize(state, Stage::Dynamics);

    // The block should have settled on ground due to dissipation.
    // The force generated by contact then should sum to body weight
    // vertically and zero horizontally when the forces are totaled
    ...
    //cout << "gfrc = " << gfrc << endl;
    //cout << "tfrc = " << tfrc << endl;
    ASSERT_EQUAL(tfrc[0], -gfrc[0], 1.0e-4); // x
    ASSERT_EQUAL(tfrc[1], -gfrc[1], 1.0e-3); // y
    ...
    if (!isEqual) {
        spr.print("origElasticContact.xml");
        sprCopy->print("cloneElasticContact.xml");
    }

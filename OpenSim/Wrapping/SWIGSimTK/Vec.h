...
public:
    ...
    {
        assert(p);
        for(int i=0; i<M; ++i) d[i*STRIDE]=p[i];
    }
    ...
    {
        for(int i=0; i<M; ++i) d[i*STRIDE] += r[i];
        return *this;
    }
    ...
    TInvert invert() const {
        assert(false);    // TODO default inversion
        return TInvert();
    }
    ...
    template <class EE> Vec& operator =(const EE& e) {
        return scalarEq(e);
    }
    template <class EE> Vec& operator+=(const EE& e) {
    ...
    {
        for(int i=0; i<M; ++i) d[i*STRIDE] = ee;
        return *this;
    }
    ...
    {
        for(int i=0; i<M; ++i) d[i*STRIDE] = ee - d[i*STRIDE];
        return *this;
    }
    ...
    static Vec<M,ELT,1> getNaN() {
        return Vec<M,ELT,1>(CNT<ELT>::getNaN());
    }
#endif
    ...
    static double getDefaultTolerance() {
        return CNT<ELT>::getDefaultTolerance();
    }

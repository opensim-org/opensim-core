...
public:
    ...
    //@{
    Rotation_&  setRotationFromAngleAboutAxis( P angle, const CoordinateAxis& axis )  {
        return axis.isXAxis() ? setRotationFromAngleAboutX(angle) : (axis.isYAxis() ? setRotationFromAngleAboutY(angle) : setRotationFromAngleAboutZ(angle) );
    }
    Rotation_&  setRotationFromAngleAboutX( P angle )  {
        return setRotationFromAngleAboutX( std::cos(angle), std::sin(angle) );
    }
    Rotation_&  setRotationFromAngleAboutY( P angle )  {
        return setRotationFromAngleAboutY( std::cos(angle), std::sin(angle) );
    }
    Rotation_&  setRotationFromAngleAboutZ( P angle )  {
        return setRotationFromAngleAboutZ( std::cos(angle), std::sin(angle) );
    }
    Rotation_&  setRotationFromAngleAboutX( P cosAngle, P sinAngle )  {
        Mat33& R = *this;
        R[0][0] = 1;
        R[0][1] = R[0][2] = R[1][0] = R[2][0] = 0;
        R[1][1] = R[2][2] = cosAngle;
        R[1][2] = -(R[2][1] = sinAngle);
        return *this;
    }
    Rotation_&  setRotationFromAngleAboutY( P cosAngle, P sinAngle )  {
        Mat33& R = *this;
        R[1][1] = 1;
        R[0][1] = R[1][0] = R[1][2] = R[2][1] = 0;
        R[0][0] = R[2][2] = cosAngle;
        R[2][0] = -(R[0][2] = sinAngle);
        return *this;
    }
    Rotation_&  setRotationFromAngleAboutZ( P cosAngle, P sinAngle )  {
        Mat33& R = *this;
        R[2][2] = 1;
        R[0][2] = R[1][2] = R[2][0] = R[2][1] = 0;
        R[0][0] = R[1][1] = cosAngle;
        R[0][1] = -(R[1][0] = sinAngle);
        return *this;
    }
    //@}
    ...
    //@{
#ifndef SWIG
    Rotation_( P angle, const UnitVec3& unitVector ) {
        setRotationFromAngleAboutUnitVector(angle,unitVector);
    }
#endif
    Rotation_( P angle, const Vec3& nonUnitVector )  {
    ...
    //@{
    Rotation_&  setRotationFromAngleAboutNonUnitVector( P angle, const Vec3& nonUnitVector )  {
        return setRotationFromAngleAboutUnitVector( angle, UnitVec3(nonUnitVector) );
    }
#ifndef SWIG
    ...
    Rotation_( BodyOrSpaceType bodyOrSpace, P angle1, const CoordinateAxis& axis1, P angle2, const CoordinateAxis& axis2 )                                            {
    ...
    Rotation_( BodyOrSpaceType bodyOrSpace, P angle1, const CoordinateAxis& axis1, P angle2, const CoordinateAxis& axis2, P angle3, const CoordinateAxis& axis3 )  {
    ...
    void setRotationToBodyFixedXY( const Vec2& v)   {
        setRotationFromTwoAnglesTwoAxes(     BodyRotationSequence, v[0], XAxis, v[1], YAxis );
    }
    /// Set this Rotation_ to represent a rotation characterized by subsequent rotations of:
    /// +v[0] about the body frame's X axis,      followed by a rotation of
    ...
    void setRotationToBodyFixedXYZ( const Vec3& v)  {
        setRotationFromThreeAnglesThreeAxes( BodyRotationSequence, v[0], XAxis, v[1], YAxis, v[2], ZAxis );
    }
#ifndef SWIG
    /// Constructor for creating a rotation matrix from a quaternion.
    explicit Rotation_( const QuaternionP& q )  {
    ...
    explicit Rotation_( const Mat33& m )  {
        setRotationFromApproximateMat33(m);
    }
    ...
    //@{
#ifndef SWIG
    Rotation_( const UnitVec3& uvec, const CoordinateAxis axis )  {
    ...
    //@{
    Rotation_( const UnitVec3& uveci, const CoordinateAxis& axisi, const Vec3& vecjApprox, const CoordinateAxis& axisjApprox )  {
    ...
    Vec3  convertRotationToBodyFixedXYZ() const  {
        return convertThreeAxesRotationToThreeAngles( BodyRotationSequence, XAxis, YAxis, ZAxis );
    }
    ...
    P  getMaxAbsDifferenceInRotationElements( const Rotation_& R ) const {
        const Mat33& A = asMat33();
        const Mat33& B = R.asMat33();
        P maxDiff = 0;
    ...
    const Mat33&  asMat33() const  {
        return *static_cast<const Mat33*>(this);
    }
    Mat33         toMat33() const  {
        return asMat33();
    }
...
private:
    ...
    static Rotation_ zero() {
        return Rotation_();
    }
    static Rotation_ NaN()  {
        Rotation_ r;
        r.setRotationToNaN();
        return r;
    }
    ...
    Rotation_&  setToZero()            {
        return setRotationToIdentityMatrix();
    }
    Rotation_&  setToIdentityMatrix()  {
        return setRotationToIdentityMatrix();
    }
    Rotation_&  setToNaN()             {
        return setRotationToNaN();
    }
    static Rotation_  trustMe( const Mat33& m )  {
        return Rotation_(m,true);
    }

    // One-angle rotations.
    static Rotation_ aboutX( const P& angleInRad ) {
        return Rotation_( angleInRad, XAxis );
    }
    static Rotation_ aboutY( const P& angleInRad ) {
        return Rotation_( angleInRad, YAxis );
    }
    static Rotation_ aboutZ( const P& angleInRad ) {
        return Rotation_( angleInRad, ZAxis );
    }
    static Rotation_ aboutAxis( const P& angleInRad, const UnitVec3& axis ) {
        return Rotation_(angleInRad,axis);
    }
    static Rotation_ aboutAxis( const P& angleInRad, const Vec3& axis )     {
        return Rotation_(angleInRad,axis);
    }
    void  setToRotationAboutZ( const P& q ) {
        setRotationFromAngleAboutZ( q );
    }

    // Two-angle space-fixed rotations.
    static Rotation_ aboutXThenOldY(const P& xInRad, const P& yInRad) {
        return Rotation_( SpaceRotationSequence, xInRad, XAxis, yInRad, YAxis );
    }
    static Rotation_ aboutYThenOldX(const P& yInRad, const P& xInRad) {
        return Rotation_( SpaceRotationSequence, yInRad, YAxis, xInRad, XAxis );
    }
    static Rotation_ aboutXThenOldZ(const P& xInRad, const P& zInRad) {
        return Rotation_( SpaceRotationSequence, xInRad, XAxis, zInRad, ZAxis );
    }
    static Rotation_ aboutZThenOldX(const P& zInRad, const P& xInRad) {
        return Rotation_( SpaceRotationSequence, zInRad, ZAxis, xInRad, XAxis );
    }
    static Rotation_ aboutYThenOldZ(const P& yInRad, const P& zInRad) {
        return Rotation_( SpaceRotationSequence, yInRad, YAxis, zInRad, ZAxis );
    }
    static Rotation_ aboutZThenOldY(const P& zInRad, const P& yInRad) {
        return Rotation_( SpaceRotationSequence, zInRad, ZAxis, yInRad, YAxis );
    }

    // Two-angle body fixed rotations (reversed space-fixed ones).
    static Rotation_ aboutXThenNewY(const P& xInRad, const P& yInRad) {
        return Rotation_( BodyRotationSequence, xInRad, XAxis, yInRad, YAxis );
    }
    static Rotation_ aboutYThenNewX(const P& yInRad, const P& xInRad) {
        return aboutXThenOldY(xInRad, yInRad);
    }
    static Rotation_ aboutXThenNewZ(const P& xInRad, const P& zInRad) {
        return aboutZThenOldX(zInRad, xInRad);
    }
    static Rotation_ aboutZThenNewX(const P& zInRad, const P& xInRad) {
        return aboutXThenOldZ(xInRad, zInRad);
    }
    static Rotation_ aboutYThenNewZ(const P& yInRad, const P& zInRad) {
        return aboutZThenOldY(zInRad, yInRad);
    }
    static Rotation_ aboutZThenNewY(const P& zInRad, const P& yInRad) {
    ...
    explicit Rotation_( const UnitVec3& uvecZ )  {
    ...
    Rotation_( const Vec3& x, const Vec3& yish )  {
        setRotationFromTwoAxes( UnitVec3(x), XAxis, yish, YAxis );
    }

    /// Set this Rotation_ to represent the same rotation as the passed-in quaternion.
    void setToQuaternion( const QuaternionP& q )  {
        setRotationFromQuaternion(q);
    }
    ...
    void setToBodyFixed321( const Vec3& v)  {
        setRotationFromThreeAnglesThreeAxes( BodyRotationSequence, v[0], ZAxis, v[1], YAxis, v[2], XAxis );
    }
    void setToBodyFixed123( const Vec3& v)  {
    ...
    Vec3  convertToBodyFixed123() const  {
        return convertRotationToBodyFixedXYZ();
    }
    Vec2  convertToBodyFixed12() const   {
        return convertRotationToBodyFixedXY();
    }
    Vec2  convertToSpaceFixed12() const  {
...
public:
    ...
    Rotation_<P>&        updInvert() {
        return *reinterpret_cast<Rotation_<P>*>(this);
    }
#endif
    //@}
...
Rotation_<P>::operator=(const InverseRotation_<P>& R)  {
    static_cast<Mat<3,3,P>&>(*this)  = R.asMat33();
    return *this;
}
template <class P> inline Rotation_<P>&

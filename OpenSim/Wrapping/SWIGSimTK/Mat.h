...
public:
    ...
    {
        for (int j=0; j<N; ++j) (*this)(j) = ELT(0);
        diag()=e;
    }
    ...
    {
        assert(p);
        for(int i=0; i<M; ++i) (*this)[i]=&p[i*N];
    }
    ...
    const THerm& transpose()    const {
        return *reinterpret_cast<const THerm*>(this);
    }
    THerm&       updTranspose()       {
        return *reinterpret_cast<THerm*>(this);
    }
    ...
    const TReal& real() const {
        return *reinterpret_cast<const TReal*>(this);
    }
    TReal&       real()       {
        return *reinterpret_cast<      TReal*>(this);
    }
    ...
    const TDiag& diag() const {
        return *reinterpret_cast<const TDiag*>(d);
    }
    /// Select main diagonal (of largest leading square if rectangular) and
    /// return it as a writable view (as a Vec) of the diagonal elements
    /// of this Mat.
    TDiag&       updDiag()    {
        return *reinterpret_cast<TDiag*>(d);
    }
    ...
    template <class EE> Mat& operator =(const EE& e) {
        return scalarEq(e);
    }
    template <class EE> Mat& operator+=(const EE& e) {
        return scalarPlusEq(e);
    }
    template <class EE> Mat& operator-=(const EE& e) {
        return scalarMinusEq(e);
    }
    template <class EE> Mat& operator*=(const EE& e) {
        return scalarTimesEq(e);
    }
    template <class EE> Mat& operator/=(const EE& e) {
    ...
    {
        for(int j=0; j<N; ++j) (*this)(j).scalarTimesEq(ee);
        return *this;
    }
    template <class EE> Mat& scalarTimesEqFromLeft(const EE& ee)
    {
        for(int j=0; j<N; ++j) (*this)(j).scalarTimesEqFromLeft(ee);
        return *this;
    }

    template <class EE> Mat& scalarDivideEq(const EE& ee)
    {
        for(int j=0; j<N; ++j) (*this)(j).scalarDivideEq(ee);
        return *this;
    }
    template <class EE> Mat& scalarDivideEqFromLeft(const EE& ee)
    {
        for(int j=0; j<N; ++j) (*this)(j).scalarDivideEqFromLeft(ee);
        return *this;
    }
#endif
    void setToNaN() {
    ...
    static const Mat& getAs(const ELT* p)  {
        return *reinterpret_cast<const Mat*>(p);
    }
    static Mat&       updAs(ELT* p)        {
        return *reinterpret_cast<Mat*>(p);
    }
    ...
    static double getDefaultTolerance() {
        return MinDim*CNT<ELT>::getDefaultTolerance();
    }
#ifndef SWIG
...
operator*(int l, const Mat<M,N,E,CS,RS>& r) {
    return r * (typename CNT<E>::Precision)l;
}
...
operator*(const Mat<M,N,E,CS,RS>& l, const negator<R>& r) {
    return l * (typename negator<R>::StdNumber)(R)r;
}
template <int M, int N, class E, int CS, int RS, class R> inline
typename Mat<M,N,E,CS,RS>::template Result<typename negator<R>::StdNumber>::Mul
operator*(const negator<R>& l, const Mat<M,N,E,CS,RS>& r) {
    return r * (typename negator<R>::StdNumber)(R)l;
}
...
operator/(const negator<R>& l, const Mat<M,N,E,CS,RS>& r) {
    return (typename negator<R>::StdNumber)(R)l/r;
}

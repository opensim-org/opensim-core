...
public:
    ...
    Transform_&  setToZero()  {
        R_BF.setRotationToIdentityMatrix();
        p_BF = P(0);
        return *this;
    }
    ...
    const InverseTransform_<P>&  invert() const  {
        return *reinterpret_cast<const InverseTransform_<P>*>(this);
    }
    ...
    InverseTransform_<P>&  updInvert()  {
        return *reinterpret_cast<InverseTransform_<P>*>(this);
    }
    ...
    const typename Rotation_<P>::ColType&  x() const  {
        return R().x();
    }
    /// Return a read-only reference to the y direction (unit vector)
    /// of the F frame, expressed in the B frame.
    const typename Rotation_<P>::ColType&  y() const  {
        return R().y();
    }
    /// Return a read-only reference to the z direction (unit vector)
    /// of the F frame, expressed in the B frame.
    const typename Rotation_<P>::ColType&  z() const  {
        return R().z();
    }
#endif
    ...
    const Mat<3,4,P>&  asMat34() const  {
        return Mat<3,4,P>::getAs(reinterpret_cast<const P*>(this));
    }
...
public:
    ...
    const Transform_<P>&  invert() const  {
        return *reinterpret_cast<const Transform_<P>*>(this);
    }
    Transform_<P>&  updInvert()           {
        return *reinterpret_cast<Transform_<P>*>(this);
    }
    ...
    const typename InverseRotation_<P>::ColType&  x() const  {
        return R().x();
    }
    const typename InverseRotation_<P>::ColType&  y() const  {
        return R().y();
    }
    const typename InverseRotation_<P>::ColType&  z() const  {
        return R().z();
    }
    ...
    Mat<3,4,P>  toMat34() const  {
        return Transform_<P>(*this).asMat34();
    }

    /// Return the equivalent 4x4 transformation matrix.
    Mat<4,4,P>  toMat44() const  {
        return Transform_<P>(*this).toMat44();
    }
...
operator*(const Transform_<P>& X_BF, const Vec<4,P,S>& a_F) {
    ...
    Vec<4,P> out;
    if( a_F[3] == 0 ) {
        Vec<3,P>::updAs(&out[0]) = X_BF.xformFrameVecToBase(v_F);
        out[3] = 0;
    }
    else              {
        Vec<3,P>::updAs(&out[0]) = X_BF.shiftFrameStationToBase(v_F);
        out[3] = 1;
    }
    return out;
}
...
template <class P, int S> inline Vec<4,P>
operator*(const Transform_<P>& X_BF,        const Vec<4,negator<P>,S>& s_F)  {
    return X_BF*Vec<4,P>(s_F);
}
template <class P, int S> inline Vec<4,P>
...
/// @relates Transform_
template <class P> inline Transform_<P>
operator*(const Transform_<P>& X1,        const Transform_<P>& X2)         {
    return X1.compose(X2);
}
template <class P> inline Transform_<P>
operator*(const Transform_<P>& X1,        const InverseTransform_<P>& X2)  {
    return X1.compose(X2);
}
template <class P> inline Transform_<P>
operator*(const InverseTransform_<P>& X1, const Transform_<P>& X2)         {
    return X1.compose(X2);
}
template <class P> inline Transform_<P>
operator*(const InverseTransform_<P>& X1, const InverseTransform_<P>& X2)  {
    return X1.compose(X2);
}

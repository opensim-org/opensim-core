/* -------------------------------------------------------------------------- *
 *                OpenSim:  examplePolynomialPathFitter.cpp                   *
 * -------------------------------------------------------------------------- *
...
int main() {
    ...
    // Set the model.
    //
    // The model should contain path-based force objects (e.g., Muscles) that
    // use geometry-based paths (e.g., GeometryPath) to model path lengths and
    // moment arms. The fitter will create a set of FunctionBasedPaths that use
    ...
    //
    // The fitter will randomly sample around the coordinate values provided in
    // the table to generate model configurations for which to compute path
    // lengths and moment arms. In general, it is up to the user to decide how
    // many sample points are needed to adequately cover the range of motion of
    // the model's coordinates. In this case, the table has many more rows than
    // are needed for the fitter to generate a good fit, so we will remove some
    // of the rows to speed up the fitting process.
    TimeSeriesTable values("coordinates.sto");
    auto times = values.getIndependentColumn();
    ...
    // coordinate values. An amplitude of 0.5 keeps the values within the
    // coordinate range of the MTP joints in the model.
    double amplitude = 0.5;
    double angular_frequency = 10.0; // rad/s
    double phase = 0.0;
    Sine sine(amplitude, angular_frequency, phase);
    SimTK::Vector mtp_values(static_cast<int>(values.getNumRows()), 0.0);
    SimTK::Vector time(1, 0.0);
    for (int i = 0; i < static_cast<int>(values.getNumRows()); i++) {
        time.set(0, times[i]);
    ...
    // model.
    fitter.setOutputDirectory("results");
    ...
    fitter.appendCoordinateSamplingBounds(
        "/jointset/hip_r/hip_flexion_r", SimTK::Vec2(-15, 15));
    fitter.appendCoordinateSamplingBounds(
        "/jointset/hip_l/hip_flexion_l", SimTK::Vec2(-15, 15));
    ...
    std::string functionBasedPathsFile =
            "results/" + model.getName() + "_FunctionBasedPathSet.xml";

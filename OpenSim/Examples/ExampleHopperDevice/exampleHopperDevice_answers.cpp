...
static const std::string hopperHeightCoord{"/Dennis/jointset/slider/yCoord"};
...
static const std::string thighAttachment{
        "/Dennis/bodyset/thigh/deviceAttachmentPoint"};
static const std::string shankAttachment{
        "/Dennis/bodyset/shank/deviceAttachmentPoint"};
...
static const std::string vastus{"/Dennis/forceset/vastus"};
...
void addConsoleReporterToHopper(Model& hopper)
{
    ...
    reporter->addToReport(
        hopper.getComponent("/Dennis/forceset/vastus").getOutput("activation"));

    #pragma endregion
    #pragma region Step1_TaskB_solution

    reporter->addToReport(
        hopper.getComponent("/Dennis/jointset/knee/kneeFlexion").
...
void run(bool showVisualizer, double finalTime)
{
    ...
    if (true)
    {
        ...
        hopper.getComponent("/Dennis/bodyset/thigh").printOutputInfo(false);

...
#define GET_VERSION_STRING  \
    MAKE_VERSION_STRING(OPENSIM_WRAPS_MAJOR_VERSION,  \
                        OPENSIM_WRAPS_MINOR_VERSION,  \
                        OPENSIM_WRAPS_BUILD_VERSION)

#define GET_COPYRIGHT_STRING \
    MAKE_COPYRIGHT_STRING(OPENSIM_WRAPS_COPYRIGHT_YEARS, \
                          OPENSIM_WRAPS_AUTHORS)

#define GET_AUTHORS_STRING \
    MAKE_STRING(OPENSIM_WRAPS_AUTHORS)

#define GET_LIBRARY_STRING \
    MAKE_STRING(OPENSIM_WRAPS_LIBRARY_NAME)

#define GET_TYPE_STRING \
    MAKE_STRING(OPENSIM_WRAPS_TYPE)
...
void opensim_version_wraps(int* major, int* minor, int* build) {
    ...
    if (major) *major = OPENSIM_WRAPS_MAJOR_VERSION;
    if (minor) *minor = OPENSIM_WRAPS_MINOR_VERSION;
    if (build) *build = OPENSIM_WRAPS_BUILD_VERSION;
...
void opensim_about_wraps(const char* key, int maxlen, char* value) {

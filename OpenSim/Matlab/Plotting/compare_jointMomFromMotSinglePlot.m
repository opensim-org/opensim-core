function [] = compare_jointMomFromMotSinglePlot(subject, fnames, tInfo, figHandleArray, figContents, ref_dataFormat)
...
pLineColor = { {[1 1 0.75], [0.75 0.75 0]}, ...
               {[1 0.75 1], [0.75  0.5 0]}, ...
...
for figNum = 1:length(figHandleArray)
    ...
    for i = 1:length(tInfo)
        ...
        [q, time, timeRange, ictoEvents] = get_commonComparisonData('Joint Moments vs Time', subject, fnames, tInfo{i});
        ...
        for plotIndex = 1:nSubPlots
            ...
            if timeAxisIsPercentGc
                overlay_jointMomentsFromMotSinglePlot(q(i), ...
                    figContents(figNum).qPlotLabel{plotIndex}, ...
                    percent{i}, pLineColor{i});
            else
                overlay_jointMomentsFromMotSinglePlot(q(i), ...
                    figContents(figNum).qPlotLabel{plotIndex}, ...
                    time{i}, pLineColor{i});
        ...
        % Format figure.
        if timeAxisIsPercentGc
            format_jointMomFromMotSinglePlot(fnames, percentRange, tInfo{i}, ...
            ...
            format_jointMomFromMotSinglePlot(fnames, timeRange, tInfo{i}, ...
                        figContents(figNum).subplotTitle, ...
                        figContents(figNum).subplotAxisLabel, ...
                        figContents(figNum).subplotRange, ...
                        figContents(figNum).subplotTick);

function [] = compare_jointMomFromMot(subject, fnames, plotLiteratureData, tInfo, figHandleArray, ref_dataFormat)
...
% Note: if plotLiteratureData is true, joint moments from literature will
% be plotted, but note that there seems to be some problem with the
% normalization of the joint moments, i.e. from each literature source, it
% seems unclear exactly how the joint moments were normalized, so anyone
% using this plotting method may want to look up the original sources of
% literature data below, and modify the code below to correct for any
% normalization so the plotting of literature data against simulated data
% is more correct.
%
% - Chand John

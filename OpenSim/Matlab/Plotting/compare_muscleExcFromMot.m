function [] = compare_muscleExcFromMot(subject, fnames, tInfo, figHandleArray, ref_dataFormat)
% Purpose:  Creates figure windows specified by figHandleArray and, 
%           for each file in fnames, overlays plots of:
%           (1) CMC-generated muscle excitations vs time,
%                   for muscles specified in ref_muscleExcPlotLabels()
...
[q, time, timeRange, ictoEvents] = get_commonComparisonData('Muscle Excitations vs Time', subject, fnames, tInfo);
...
figContents = ref_muscleExcPlotLabels;
...
for figNum = 1:length(figHandleArray)
    ...
    titleString = sprintf('%s%s%s%3.2f%s', ...
        'Comparison of Muscle Excitations:  Subject ', subject, ...
        ', ', tInfo.speed, ' m/s');
    Suptitle(titleString);      % MATLAB m-file for adding "super title"
end

% Query user.
done = printmenu(figHandleArray, [subject '_muscleExc']);
return;

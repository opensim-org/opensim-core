function plot_multipleSmallFiguresAndCurvesFromMotOrStoFiles( plotSettings )
%
...
for i = 1 : length( plotSettings.curveSourceFiles )
    ...
    for j = 1 : length( plotSettings.figureNumbers )
        ...
        % file.
        columnLabelsForCurvesInThisFigure = ...
            plotSettings.curveSourceColumnLabels{ i, j };
        for curveNum = 1 : length( columnLabelsForCurvesInThisFigure )
            columnLabelsForThisCurve = ...
                columnLabelsForCurvesInThisFigure{ curveNum };
            dataIndices = find( strcmpi( ...
                columnLabelsForThisCurve, q.labels ) );
            ...
            p = line( time, data, 'Parent', c( axesIndex ) );
            set( p, ...
                'LineStyle', plotSettings.curveStyles{ i, curveNum }, ...
                'LineWidth', plotSettings.curveWidths{ i, curveNum }, ...
                'Color',     plotSettings.curveColors{ i, curveNum } );
        end
    end
...
% Set left axis to be the one on top in each figure.
for j = 1 : length( plotSettings.figureNumbers )
    % Set current figure.
    figure( plotSettings.figureNumbers(j) );
    % Get handle for left axis.
    ax = get( gcf, 'Children' );
    if length( ax ) > 1
        leftAxis = ax( end );
    else
...
% Remove all plot labels, titles, ticks, and tick labels.  Also make the
% axes take up the whole figure.
for j = 1 : length( plotSettings.figureNumbers )
    % Set current figure.
    figure( plotSettings.figureNumbers(j) );
    % Remove the title.
    ...
    for i = 1 : length( ax )
        a = ax(i);
        % Make axes take up the whole figure.
        set( a, 'Units', 'inches' );
        set( a, 'Position', [ 0 0 0.99 0.25 ] );
        % Turn off all axis lines, labels, ticks, and tick labels.
        axis( a, 'off' );
    end
    % Shrink figure.
    set( gcf, 'Position', [ figPos(1) figPos(2) 0.99 0.25 ] );
end

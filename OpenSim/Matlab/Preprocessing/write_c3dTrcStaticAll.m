function mkrMatrix = write_c3dTrcStaticAll(mStatic, cStatic, fname)
...
label{21} = {'heel_r',          'RHEE'};
label{22} = {'heel_l',          'LHEE'};
label{23} = {'sternalnotch',    'STRN'};
label{24} = {'xyphoidprocess',  'XYPH'};
label{25} = {'neck',            'N'};
label{26} = {'subclavicle_r',   'RAC'};
label{27} = {'subclavicle_l',   'LAC'};
anatomicalIndex = length(label);

label{28} = {'toe_r',           'RTOE'};        % Tracking Markers
label{29} = {'toe_l',           'LTOE'};
label{30} = {'thighwand_r',     'RTHI'};
label{31} = {'thighwand_l',     'LTHI'};
label{32} = {'upperleg1_r',     'RUL1'};
label{33} = {'upperleg1_l',     'LUL1'};
label{34} = {'upperleg2_r',     'RUL2'};
label{35} = {'upperleg2_l',     'LUL2'};
label{36} = {'shankwand_r',     'RTIB'};
label{37} = {'shankwand_l',     'LTIB'};
label{38} = {'lowerleg1_r',     'RLL1'}; 
label{39} = {'lowerleg1_l',     'LLL1'};

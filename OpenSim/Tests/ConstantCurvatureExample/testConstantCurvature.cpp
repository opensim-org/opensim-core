...
void testJacobians1() {
    ...
    ASSERT(diffJ.norm() < 1e-9, __FILE__, __LINE__, "Jacobian didn't match expected value");
    ...
    Mat63 Jdot = ConstantCurvatureJoint::getConstantCurveJacobianDerivWrtTime(q, qDot, d);

    Mat63 diffJdot = Jdot - expectedJacobianTimeDeriv;
    ASSERT(diffJdot.norm() < 1e-9, __FILE__, __LINE__, "Jacobian time deriv didn't match expected value");
}
...
int main() {
    ...
    for (int i = 0; i < numBodies; i++) {
      OpenSim::Body* body = new OpenSim::Body(
              "body_"+std::to_string(i), 1, Vec3(0), Inertia(0.1, 0.1, 0.1));

      model.addBody(body);
      bodies.push_back(body);

      // Attach an ellipsoid to a frame located at the center of each body.
      PhysicalOffsetFrame* visualCenter = new PhysicalOffsetFrame(
              "body_viz_center_" +std::to_string(i), *body, Transform(Vec3(0)));
      ...
      if (i > 0) {
        joint = new ConstantCurvatureJoint(
              "ccj" + std::to_string(i), *(bodies[i-1]), *(bodies[i]), Vec3(0, 0, 0), 1.0);
      }
      else {
        joint = new ConstantCurvatureJoint(
              "ccj" + std::to_string(i), model.getGround(), *(bodies[i]), Vec3(0, 0, 0), 1.0);
      }
      joint->updCoordinate(ConstantCurvatureJoint::Coord::RotationX).setName("joint_"+std::to_string(i)+"_x");
      joint->updCoordinate(ConstantCurvatureJoint::Coord::RotationZ).setName("joint_"+std::to_string(i)+"_z");
      joint->updCoordinate(ConstantCurvatureJoint::Coord::RotationY).setName("joint_"+std::to_string(i)+"_y");

      SpringGeneralizedForce* springForceX = new SpringGeneralizedForce("joint_"+std::to_string(i)+"_x");
      ...
      SpringGeneralizedForce* springForceZ = new SpringGeneralizedForce("joint_"+std::to_string(i)+"_z");
      ...
      SpringGeneralizedForce* springForceY = new SpringGeneralizedForce("joint_"+std::to_string(i)+"_y");
    ...
    reporter->addToReport(
            static_cast<ConstantCurvatureJoint*>(joints[0])->getCoordinate(ConstantCurvatureJoint::Coord::RotationX)
                    .getOutput("value"),"X");
    reporter->addToReport(
            static_cast<ConstantCurvatureJoint*>(joints[0])->getCoordinate(ConstantCurvatureJoint::Coord::RotationZ)
                    .getOutput("value"),"Z");
    reporter->addToReport(
            static_cast<ConstantCurvatureJoint*>(joints[0])->getCoordinate(ConstantCurvatureJoint::Coord::RotationY)
                    .getOutput("value"),"Y");
    reporter->addToReport(
            static_cast<ConstantCurvatureJoint*>(joints[0])->getCoordinate(ConstantCurvatureJoint::Coord::RotationX)
                    .getOutput("speed"),"vX");
    ...
    static_cast<ConstantCurvatureJoint*>(joints[0])->getCoordinate(ConstantCurvatureJoint::Coord::RotationZ).setValue(state, 0.01);
    // static_cast<ConstantCurvatureJoint*>(joints[0])->getCoordinate(ConstantCurvatureJoint::Coord::RotationX).setValue(state, 0.001);

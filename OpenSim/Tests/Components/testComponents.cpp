...
{
    INFO("Test if " << p->getConcreteClassName()
                    << " works in a higher-level aggregate Model");
    ...
    while (sub) {
        ...
        for (const auto& socketName : sub->getSocketNames()) {
            ...
            INFO("Socket '" << socket.getName()
                            << "' has dependency on: " << dependencyTypeName);
    ...
    for (auto& sub : model.updComponentList()) {
        for (const auto& inputName : sub.getInputNames()) {
            ...
            INFO("Input '" << input.getName() << "' has dependency on: Output<"
                           << dependencyTypeName << ">");
    ...
    try{
        ...
        INFO("testComponents::" << instance.getConcreteClassName()
                                << " unable to connect to model:");
        INFO(x.what());
        INFO("Error is likely due to " << instance.getConcreteClassName()
                                       << " having structural dependencies "
                                          "that are not specified as Sockets.");
    }
    ...
    for (const auto& entry : instance.getOutputs()) {
        ...
        INFO("Testing Output " << thisName << ", dependent on "
                               << thisOutput->getDependsOnStage().getName());
        ...
        INFO("Component " << instance.getConcreteClassName() << ", output: "
                          << thisOutput->getValueAsString(state));
    }
}
...
{
    INFO("\n**********************************************************");
    INFO("* Testing " << instanceToTest.getConcreteClassName());
    INFO("**********************************************************");

...
TEST_CASE("Test property bounds") {
    {
...
TEST_CASE("Test MuscleFirstOrderDynamicModel") {
    /**
     * Test conditions:
     ...
     * 4. Ensure that the fall time is about 2.25 time constants.
     */
    int pts = 100;
    ...
    SECTION("Activation bounds are respected") {
        // Check bounds on the step response
        ...
        INFO("PASSED: with a tolerance of "
                << std::fixed << std::setprecision(16) << SimTK::Eps * 1e16
                << "e-16");
        CHECK(boundsRespected);
    }

    SECTION("Rise and fall time") {
        std::stringstream msg;
        msg << "Testing: 10%-90% Rise time 3*tau_act +/- 10%"<<endl;
        ...
        INFO("PASSED: with a normalized 10-90 percent rise time of "
                << normRiseTime
    ...
    SECTION("Continuity of d/dt activation w.r.t. excitation and constant activation") {
        for(int i=0;i<actV.size();i++){
        ...
        INFO("PASSED: with an allowable error of minTol of "
                << minTol << " or " << taylorMult
                << " x the next Taylor series term. "
                << "These values are reasonable given the elbow in the "
                   "function and "

...
class ExponentialCoordinateForce:
    ...
    def calc_force(self, q):
        return self.limit_forces[0]*np.exp(self.shape_factors[0]*(self.coordinate_limits[0] - q)) + \
...
shape_factors = np.array([100.0, 25.0])
force = ExponentialCoordinateForce(coordinate_limits, limit_forces, shape_factors)
force_values = force.calc_force(q)
plt.plot(q, force_values, color='k', lw=2.5)

# plot vertical lines at theta and phi
plt.axvline(x=coordinate_limits[0], color='r', linestyle='--')
plt.axvline(x=coordinate_limits[1], color='r', linestyle='--')
plt.axvline(x=0, color='k', linestyle='--', lw=0.5)
plt.axhline(y=0, color='k', linestyle='--', lw=0.5)
plt.plot(coordinate_limits[0], limit_forces[0], 'ko', ms=7)
plt.plot(coordinate_limits[1], -limit_forces[1], 'ko', ms=7)

plt.text(0.9*coordinate_limits[0], 0.95*limit_forces[0],
        f'({coordinate_limits[0]:.1f}, {limit_forces[0]})', color='k', fontsize=12)
plt.text(0.4*coordinate_limits[1], -1.1*limit_forces[1],
        f'({coordinate_limits[1]:.1f}, {-limit_forces[1]})', color='k', fontsize=12)

plt.text(2.1*coordinate_limits[0], 0.4*limit_forces[0],
        r'$50 e^{-100(x+0.2)}$', color='k', fontsize=14)

plt.text(1.2*coordinate_limits[1], -2.5*limit_forces[1],
        r'$-20 e^{25(x-0.3)}$', color='k', fontsize=14)
plt.xlim(-0.5, 0.6)
plt.ylim(-100, 100)
plt.xlabel('coordinate value (rad or m)', fontsize=12)
plt.ylabel('limit force (N or Nm)', fontsize=12)
